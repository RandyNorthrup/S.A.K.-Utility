...
- **Tests** - Added deterministic chat-title generation/redaction, saved assistant response-id restoration, result-bubble copy behavior, session-role persistence, workflow role-diversity checks, exact context-window usage smoke coverage, live Responses API tool-loop coverage, and expanded multi-agent workflow evals. Current suite: 133 registered CTest tests.
