...
void RegistrySnapshotEngine::enumerateKeys(
    ...
    QSet<QString>& output, int maxDepth)
{
    if (maxDepth <= 0) {
        return;
    }
    ...
    for (DWORD i = 0; i < subkey_count; ++i) {
        ...
        if (child_name.startsWith("Classes", Qt::CaseInsensitive)
            && hiveName == "HKLM" && subkey == "SOFTWARE") {
            continue;  // HKLM\SOFTWARE\Classes is huge
        }

        QString child_path = subkey.isEmpty() ? child_name
                                               : subkey + "\\" + child_name;
