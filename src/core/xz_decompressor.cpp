...
bool XzDecompressor::initStream()
{
    ...
    if (ret != LZMA_OK) {
        m_lastError = QString("Failed to initialize lzma: error code %1")
...
XzDecompressor::StepResult XzDecompressor::decompressStep()
{
    ...
    if (ret == LZMA_STREAM_END) return StepResult::stream_end;
    if (ret != LZMA_OK) {
        m_lastError = QString("Decompression error: lzma error code %1")
            .arg(static_cast<int>(ret));
