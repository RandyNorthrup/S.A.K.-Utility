...
void PortScanner::scan(const ScanConfig& config)
{
    ...
    for (int i = 0; i < ports.size(); ++i) {
        ...
        auto result = scanPort(config.target, ports[i],
                                config.timeoutMs, config.grabBanners);
...
QString PortScanner::grabBannerData(const QString& target, uint16_t port,
                                     int timeoutMs)
{
    QTcpSocket socket;
    socket.connectToHost(target, port);
    ...
    return {
        ...
        {QStringLiteral("Top 100"),
         {7, 9, 13, 21, 22, 23, 25, 26, 37, 53, 79, 80, 81, 88, 106,
          ...
          8888, 9100, 9999, 10000, 32768, 49152, 49153, 49154}},
    };
}
