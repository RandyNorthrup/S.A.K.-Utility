...
HardwareInventoryScanner::HardwareInventoryScanner(QObject* parent)
    : QObject(parent)
{
}
...
QVector<QVariantMap> HardwareInventoryScanner::wmiQuery(
    const QString& wmi_class,
    ...
    int timeout_ms)
{
    ...
    if (ps.exitCode() != 0) {
        logError("WMI query failed for class {} (exit code {})",
                 wmi_class.toStdString(), ps.exitCode());
        return {};
    }
    ...
    if (parse_error.error != QJsonParseError::NoError) {
        logError("WMI JSON parse error for class {}: {}",
                 wmi_class.toStdString(), parse_error.errorString().toStdString());
        return {};
    }
...
CpuInfo HardwareInventoryScanner::queryCpu()
{
    ...
    switch (arch) {
        ...
        case 12: info.architecture = "ARM64"; break;
    ...
    logInfo("CPU detected: {} ({} cores / {} threads)",
            info.name.toStdString(), info.cores, info.threads);
...
MemorySummary HardwareInventoryScanner::queryMemory()
{
    ...
    const auto results = wmiQuery("Win32_PhysicalMemory", {
        "Manufacturer", "PartNumber", "Capacity", "Speed",
        "SMBIOSMemoryType", "FormFactor", "DeviceLocator", "SerialNumber"
    });
    ...
    const auto array_results = wmiQuery("Win32_PhysicalMemoryArray", {
        "MemoryDevices"
    });
...
QVector<StorageDeviceInfo> HardwareInventoryScanner::queryStorage()
{
    ...
    for (const auto& disk : results) {
        ...
        if (dev.interface_type == "NVMe" ||
            dev.model.contains("SSD", Qt::CaseInsensitive) ||
...
MotherboardInfo HardwareInventoryScanner::queryMotherboard()
{
    ...
    // BaseBoard
    const auto boards = wmiQuery("Win32_BaseBoard", {
    ...
    logInfo("Motherboard: {} {}",
            info.manufacturer.toStdString(), info.product.toStdString());
    return info;
}
...
BatteryInfo HardwareInventoryScanner::queryBattery()
{
    ...
    if (GetSystemPowerStatus(&power_status)) {
        ...
        switch (power_status.ACLineStatus) {
            case 0:  info.status = "Discharging"; break;
            case 1:  info.status = "Charging";    break;
    ...
    if (!batteries.isEmpty()) {
        ...
        if (info.design_capacity > 0 && info.full_charge_capacity > 0) {
            info.health_percent = (static_cast<double>(info.full_charge_capacity) /
                                   static_cast<double>(info.design_capacity)) * 100.0;
        }
    }

    if (info.present) {
        logInfo("Battery: {} — {:.1f}% health",
                info.name.toStdString(), info.health_percent);
    }
...
{
    const auto results = wmiQuery("Win32_OperatingSystem", {
    ...
    logInfo("OS: {} (Build {})", m_inventory.os_name.toStdString(),
