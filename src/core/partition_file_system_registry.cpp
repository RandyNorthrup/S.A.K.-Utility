...
PartitionFileSystemCapability extCapability(const QString& id) {
    return {.id = id,
            .display_name = id,
            .support_level =
                QStringLiteral("Detection plus manifest-gated e2fsprogs tool support"),
            .aliases = {id},
            .available_actions = {QStringLiteral("Read-only signature detection"),
                                  QStringLiteral("Read-only directory browse and selected-file extract with original S.A.K. parser"),
                                  QStringLiteral("Read-only bounded recursive directory export"),
                                  QStringLiteral("Read-only e2fsck check when manifest-approved"),
                                  QStringLiteral(
                                      "Confirmed ext format and repair through Pending Operations"),
                                  QStringLiteral(
                                      "Confirmed ext grow through Pending Operations and resize2fs"),
                                  QStringLiteral(
                                      "Confirmed ext shrink with e2fsck, resize2fs, and partition shrink")},
            .blocked_actions = {QStringLiteral(
                "XFS/Btrfs/APFS write workflows and HFS+ file writes are not enabled by ext tool approval")},
...
PartitionFileSystemCapability btrfsCapability() {
    return {.id = QStringLiteral("btrfs"),
            ...
            .available_actions = {QStringLiteral("Read-only signature detection"),
                                  ...
                                  QStringLiteral(
                                      "Original read-only superblock metadata consistency check")},
            .blocked_actions = {
                QStringLiteral("Deep btrfs check requires manifest-approved btrfs"),
                QStringLiteral("Repair remains disabled until destructive VM/lab certification "
...
PartitionFileSystemCapability hfsCapability(const QString& id, const QString& displayName) {
    return {.id = id,
            ...
            .available_actions = {
                ...
                QStringLiteral(
                    "Read-only catalog browse and selected data-fork extract with original S.A.K. parser"),
                ...
                QStringLiteral("Read-only bounded recursive directory export with resource-fork sidecars"),
                QStringLiteral(
                    "Image-only same-size HFS+ data-fork overwrite with explicit certification evidence"),
                QStringLiteral(
                    "Staged raw-partition HFS+ data/resource-fork replacement and truncate through Pending Operations"),
                QStringLiteral(
                    "Staged raw-partition HFS+ bounded data/resource-fork allocation growth through Pending Operations"),
                QStringLiteral(
                    "Staged raw-partition HFS+ empty-file create/delete with catalog read-back verification"),
                QStringLiteral(
                    "Staged raw-partition HFS+ bounded file create with data-fork allocation and read-back verification"),
                QStringLiteral(
                    "Staged raw-partition HFS+ allocated-file delete with allocation bitmap release and optional released-block zeroing"),
                QStringLiteral(
                    "Staged raw-partition HFS+ empty-folder create/delete with catalog read-back verification"),
                QStringLiteral(
                    "Staged raw-partition HFS+ bounded folder-tree delete with allocation bitmap release"),
                QStringLiteral(
                    "Staged raw-partition HFS+ inline attribute replacement through Pending Operations"),
                QStringLiteral(
                    "Staged raw-partition HFS+ fork-backed attribute replacement within allocated blocks"),
                QStringLiteral(
                    "Staged raw-partition HFS+ bounded fork-backed attribute allocation growth through Pending Operations"),
...
PartitionFileSystemCapability apfsCapability() {
    return {.id = QStringLiteral("apfs"),
            .display_name = QStringLiteral("APFS"),
            .support_level = QStringLiteral(
                "Detection, original read-only browse/extract, and one-chunk generated APFS format/repair/root-file mutation"),
            .aliases = {QStringLiteral("APFS"), QStringLiteral("apfs")},
            .available_actions = {QStringLiteral("Read-only signature detection"),
                                  QStringLiteral(
                                      "Read-only container checkpoint ring, object-map, volume-OID, and volume-superblock candidate metadata inspection"),
                                  QStringLiteral(
                                      "Original read-only container metadata consistency check"),
                                  QStringLiteral(
                                      "Original read-only APFS volume browse, selected-file extraction, and bounded recursive export"),
                                  QStringLiteral(
                                      "Confirmed S.A.K. one-spaceman-chunk generated APFS create and format through Pending Operations (64 MiB through 128 MiB targets)"),
                                  QStringLiteral(
                                      "Confirmed one-spaceman-chunk generated APFS metadata-checksum repair through Pending Operations"),
                                  QStringLiteral(
                                      "Confirmed one-spaceman-chunk generated APFS root-file write, patch, and delete through Pending Operations")},
            .blocked_actions = {
                QStringLiteral(
                    "Arbitrary existing Apple APFS mutation remains blocked by generated-layout guards"),
                QStringLiteral(
                    "Generated APFS targets larger than one spaceman chunk require multi-CIB spaceman support and Apple fsck validation"),
                QStringLiteral(
                    "Encrypted/compressed files, file writes on arbitrary Apple APFS, and resize remain blocked")},
...
PartitionFileSystemCapability linuxSwapCapability() {
    return {.id = QStringLiteral("linux-swap"),
            ...
            .aliases = {QStringLiteral("Linux swap"), QStringLiteral("linux-swap"), QStringLiteral("swap")},
            .available_actions = {QStringLiteral("Read-only signature detection"),
...
PartitionFileSystemCapability PartitionFileSystemRegistry::capabilityFor(
    ...
    return {.id = value,
            ...
            .available_actions = {},
            .blocked_actions = {
                QStringLiteral("S.A.K. has no registered filesystem workflow for this label")},
            .non_native = false};
}
