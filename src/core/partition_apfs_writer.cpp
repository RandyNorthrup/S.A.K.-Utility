...
constexpr uint64_t kFletcherModulo = 0xFFFFFFFFULL;
...
constexpr uint64_t kApfsFormatGhostReservedBlocks =
    kApfsFormatGhostContainerOmapTreeBlock + 1;
...
constexpr uint64_t kApfsDeterministicUuidSalt = 0xA5A55A5AULL;
...
constexpr uint32_t kApfsObjStoragePhysical = 0x40000000;
constexpr uint32_t kApfsObjStorageEphemeral = 0x80000000;
constexpr uint32_t kApfsObjectTypeNxSuperblock = kApfsObjStorageEphemeral | 0x00000001;
constexpr uint32_t kApfsObjectTypeBtree = 0x00000002;
constexpr uint32_t kApfsObjectTypeBtreePhysical = kApfsObjStoragePhysical | 0x00000002;
constexpr uint32_t kApfsObjectTypeObjectMap = kApfsObjStoragePhysical | 0x0000000B;
constexpr uint32_t kApfsObjectTypeFs = 0x0000000D;
constexpr uint32_t kApfsObjectTypeSpaceman = kApfsObjStorageEphemeral | 0x00000005;
constexpr uint32_t kApfsObjectTypeCheckpointMap = kApfsObjStoragePhysical | 0x0000000C;
constexpr uint32_t kApfsObjectTypeReaper = kApfsObjStorageEphemeral | 0x00000011;
constexpr uint32_t kApfsObjectSubtypeExtentRef = 0x0000000F;
constexpr uint32_t kApfsObjectSubtypeSnapMeta = 0x00000010;
constexpr uint32_t kApfsObjectTypeBtreeEphemeral = kApfsObjStorageEphemeral | 0x00000002;
constexpr uint32_t kApfsObjectSubtypeFreeQueue = 0x00000009;
constexpr uint32_t kApfsObjectTypeChunkInfoBlock = kApfsObjStoragePhysical | 0x00000007;
constexpr uint32_t kApfsObjectSubtypeFsTree = 0x0000000E;
constexpr uint32_t kApfsMagicNxsb = 0x4253584E;
constexpr uint32_t kApfsMagicApsb = 0x42535041;
...
// constant).
constexpr uint64_t kApfsGeneratedTimestamp = 0x18B8305B1F328192ULL;
...
constexpr uint32_t kApfsCheckpointMapLastFlag = 0x00000001;
constexpr qsizetype kApfsNxReaperOidOffset = 0xA8;
// nx_ephemeral_info[0] packs NX_EPH_MIN_BLOCK_COUNT (8) << 32 |
// NX_MAX_FILE_SYSTEM_EPH_STRUCTS (4) << 16 | NX_EPH_INFO_VERSION_1 (1);
// fsck_apfs fails the checkpoint consistency check when it is zero.
constexpr qsizetype kApfsNxEphemeralInfoOffset = 0x520;
constexpr uint64_t kApfsNxEphemeralInfoValue = 0x0000000400040001ULL;
constexpr qsizetype kApfsReaperNextReapIdOffset = 0x20;
constexpr qsizetype kApfsReaperFlagsOffset = 0x40;
constexpr uint32_t kApfsReaperBhmFlag = 0x00000001;
...
constexpr uint32_t kApfsSpacemanBlocksPerChunk = 32768;
...
constexpr uint16_t kApfsSpacemanFqIpLimit = 1;
constexpr uint16_t kApfsSpacemanFqMainLimit = 4;
...
[[nodiscard]] bool hasSnapshotMutationRisk(const QStringList& details) {
    for (const auto& detail : details) {
        ...
        const QString countText =
            detail.mid(snapshotCountAt + kApfsSnapshotCountPrefixLength)
...
uint16_t le16(const QByteArray& bytes, qsizetype offset) {
    ...
    return qFromLittleEndian<uint16_t>(
...
uint64_t le64(const QByteArray& bytes, qsizetype offset) {
    ...
    return qFromLittleEndian<uint64_t>(
        reinterpret_cast<const uchar*>(bytes.constData() + offset));
}
...
QByteArray buildObjectMapTreeBlock(uint32_t blockSize,
    ...
    QByteArray block =
        newApfsObjectBlock(blockSize, oid, xid, kApfsObjectTypeBtreePhysical);
    writeLe32(&block, kApfsObjectSubtypeOffset, 0x0000000B);
    ...
    const qsizetype keyBytesUsed =
        static_cast<qsizetype>(mappings.size()) * kApfsObjectMapKeyBytes;
    const qsizetype valueBytesUsed =
        static_cast<qsizetype>(mappings.size()) * kApfsObjectMapValueBytes;
    writeLe16(&block, 0x2C, static_cast<uint16_t>(keyBytesUsed));
    writeLe16(&block,
              0x2E,
              static_cast<uint16_t>(valueAreaEnd - keyAreaStart - keyBytesUsed -
                                    valueBytesUsed));
    ...
    writeLe32(&block, infoOffset + kApfsBtreeInfoFlagsOffset, 0x00000012);
...
uint32_t drecNameLenAndHash(const QString& name) {
    ...
    const uint32_t nameLength =
        static_cast<uint32_t>(name.toUtf8().size() + 1) & 0x3FF;
    return ((crc & 0x3FFFFF) << 10) | nameLength;
}
...
QByteArray inodeValue(uint64_t parentId,
                      uint64_t privateId,
                      uint16_t mode,
                      const QString& name,
    ...
    const qsizetype tocBytes = kApfsXfieldHeaderBytes +
                               xfieldCount * kApfsXfieldTocEntryBytes;
    ...
    writeLe16(&value,
              ...
              static_cast<uint16_t>(namePadded +
                                    (regularFile ? kApfsDstreamMinBytes : 0)));
    ...
    if (regularFile) {
        ...
        writeLe64(&value,
                  dstream + 8,
                  ((sizeBytes + kSupportedApfsBlockSizeBytes - 1) /
...
QByteArray fileExtentKey(uint64_t privateId) {
    QByteArray key = fsKey(privateId,
                           kApfsRecordFileExtent,
                           kApfsFileExtentKeyBytes);
...
qsizetype alignApfsValueBytes(qsizetype bytes) {
    return ((bytes + kApfsXfieldAlignmentPadding) / kApfsXfieldAlignment) *
           kApfsXfieldAlignment;
}
...
void appendRootFileRecords(QVector<ApfsBtreeKeyValue>* records,
                           const ApfsRootFilePayload& file) {
    records->append({fsKey(file.fileId, kApfsRecordInode),
                     inodeValue(file.parentDirectoryId,
                                file.privateId,
    ...
    records->append({fileExtentKey(file.privateId),
                     fileExtentValue(static_cast<uint64_t>(file.data.size()),
                                     file.dataStartBlock)});
}
...
void appendRootDirectoryRecords(QVector<ApfsBtreeKeyValue>* records,
    ...
    records->append({fsKey(directory.directoryId, kApfsRecordInode),
                     inodeValue(kApfsRootDirectoryId,
                                directory.privateId,
                                kApfsModeDirectory,
...
QByteArray buildRootTreeBlock(uint32_t blockSize,
    ...
    QVector<ApfsBtreeKeyValue> records{
        ...
        {fsKey(kApfsRootDirectoryId, kApfsRecordInode),
         inodeValue(kApfsTreeRootEntityId,
                    kApfsRootDirectoryId,
                    kApfsModeDirectory,
                    QStringLiteral("root"),
                    0,
                    static_cast<int32_t>(files.size() + directories.size()))},
        {fsKey(kApfsPrivateDirectoryId, kApfsRecordInode),
         inodeValue(kApfsTreeRootEntityId,
                    kApfsPrivateDirectoryId,
                    kApfsModeDirectory | 0644,
                    QStringLiteral("private-dir"),
                    0,
                    0)}};
    ...
    const qsizetype tocLength =
        ((static_cast<qsizetype>(records.size()) * kApfsBtreeVariableTocEntryBytes + 63) /
         64) *
        64;
    writeLe16(&block, kApfsBtreeNodeTableOffsetOffset, 0);
    ...
    for (qsizetype index = 0; index < records.size(); ++index) {
        ...
        const qsizetype toc = kApfsBtreeNodeHeaderBytes +
                              index * kApfsBtreeVariableTocEntryBytes;
    ...
    writeLe16(&block,
              0x2E,
              static_cast<uint16_t>(valueAreaEnd - keyAreaStart - keyCursor -
                                    valueBackCursor));
    writeLe16(&block, 0x30, 0xFFFF);
    writeLe16(&block, 0x32, 0);
    writeLe16(&block, 0x34, 0xFFFF);
    ...
    writeLe32(&block, infoOffset + kApfsBtreeInfoFlagsOffset, 0x00000042);
...
QByteArray buildNxSuperblock(uint32_t blockSize,
    ...
    writeLe64(&block, kApfsNxEphemeralInfoOffset, kApfsNxEphemeralInfoValue);
    writeLe64(&block, kApfsNxOmapOidOffset, position.omapOid);
    writeLe32(&block, kApfsNxMaxFileSystemsOffset, 1);
...
QByteArray buildFreeQueueTreeBlock(uint32_t blockSize,
    ...
    QByteArray block = newApfsObjectBlock(blockSize, oid, kApfsFormatXid,
    ...
    const uint16_t freeLength = static_cast<uint16_t>(
        infoOffset - keyAreaStart - 16 - 8);
    ...
    writeLe32(&block, infoOffset + kApfsBtreeInfoFlagsOffset, 0x0000000E);
    writeLe32(&block, infoOffset + kApfsBtreeInfoNodeSizeOffset, blockSize);
...
QByteArray buildExtentRefTreeBlock(uint32_t blockSize,
    ...
    const qsizetype tocLength =
        std::max<qsizetype>(64,
                            ((records.size() * kApfsBtreeVariableTocEntryBytes + 63) / 64) * 64);
    writeLe32(&block, kApfsBtreeNodeCountOffset, static_cast<uint32_t>(records.size()));
    writeLe16(&block, kApfsBtreeNodeTableOffsetOffset, 0);
    writeLe16(&block, kApfsBtreeNodeTableLengthOffset, static_cast<uint16_t>(tocLength));
    const qsizetype keyAreaStart = kApfsBtreeNodeHeaderBytes + tocLength;
    ...
    for (qsizetype index = 0; index < records.size(); ++index) {
        const auto& record = records.at(index);
        const qsizetype toc =
            kApfsBtreeNodeHeaderBytes + index * kApfsBtreeVariableTocEntryBytes;
        writeLe16(&block, toc, static_cast<uint16_t>(keyCursor));
    ...
    writeLe16(&block, 0x36, 0);
    const qsizetype infoOffset = valueAreaEnd;
    writeLe32(&block, infoOffset + kApfsBtreeInfoFlagsOffset, 0x00000052);
    writeLe32(&block, infoOffset + kApfsBtreeInfoNodeSizeOffset, blockSize);
    if (!records.isEmpty()) {
...
QByteArray buildEmptyVariableTreeBlock(uint32_t blockSize,
    ...
    const qsizetype infoOffset = static_cast<qsizetype>(blockSize) - kApfsBtreeInfoBytes;
    const uint16_t freeLength = static_cast<uint16_t>(
        infoOffset - kApfsBtreeNodeHeaderBytes - 64);
...
QByteArray buildSpacemanBlock(uint32_t blockSize,
                              uint64_t blockCount,
    ...
    const uint64_t chunkCount =
        (blockCount + kApfsSpacemanBlocksPerChunk - 1) / kApfsSpacemanBlocksPerChunk;
    ...
    writeLe64(&block, kApfsSpacemanIpBlockCountOffset, kApfsFormatIpBlockCount);
    ...
    writeLe16(&block, kApfsSpacemanFqIpLimitOffset, kApfsSpacemanFqIpLimit);
    writeLe16(&block, kApfsSpacemanFqMainLimitOffset, kApfsSpacemanFqMainLimit);
...
QByteArray buildChunkInfoBlock(uint32_t blockSize,
                               uint64_t blockCount,
                               uint64_t reservedBlocks,
                               uint64_t xid,
                               uint64_t selfBlock,
    ...
    writeLe32(&block, kApfsChunkInfoCountOffset, 1);
    const qsizetype entry = kApfsChunkInfoEntriesOffset;
    writeLe64(&block, entry, xid);
    writeLe64(&block, entry + kApfsChunkInfoEntryAddrOffset, 0);
    writeLe32(&block,
              entry + kApfsChunkInfoEntryBlockCountOffset,
              static_cast<uint32_t>(blockCount));
    writeLe32(&block,
              entry + kApfsChunkInfoEntryFreeCountOffset,
              static_cast<uint32_t>(blockCount > reservedBlocks ? blockCount - reservedBlocks
                                                                : 0));
    writeLe64(&block, entry + kApfsChunkInfoEntryBitmapAddrOffset, bitmapBlock);
    stampApfsObjectBlock(&block, blockers);
    return block;
}
...
QByteArray buildObjectMapBlock(uint32_t blockSize,
                               uint64_t oid,
                               uint64_t treeBlock,
                               uint64_t xid,
                               uint64_t omapFlags,
                               QStringList* blockers) {
    QByteArray block =
        newApfsObjectBlock(blockSize, oid, xid, kApfsObjectTypeObjectMap);
...
QByteArray buildVolumeSuperblock(const ApfsVolumeSuperblockFields& fields, QStringList* blockers) {
    QByteArray block =
        newApfsObjectBlock(fields.blockSize,
                           kApfsFormatVolumeOid,
                           kApfsFormatXid,
                           kApfsObjectTypeFs);
    ...
    writeLe64(&block, 0x68, 0x1741015800000006ULL);
    writeLe64(&block, 0x70, 0x0000000200000001ULL);
    writeLe64(&block, 0x78, 0x4000000240000002ULL);
    ...
    writeLe64(&block,
              ...
              fields.allocatedBlocks);
    writeLe64(&block, kApfsVolumeOmapOidOffset, kApfsFormatVolumeOmapBlock);
    writeLe64(&block, kApfsVolumeRootTreeOidOffset, kApfsFormatRootTreeOid);
    writeLe64(&block, 0x90, kApfsFormatExtentRefTreeBlock);
    writeLe64(&block, kApfsVolumeSnapMetaTreeOidOffset, kApfsFormatSnapMetaTreeBlock);
...
QString fileSha256Hex(const QString& path, QStringList* blockers) {
    ...
    if (!device) {
        blockers->append(QStringLiteral("Unable to reopen APFS target for hashing: %1")
                             .arg(openError));
        return {};
    }
    QCryptographicHash hash(QCryptographicHash::Sha256);
    if (!hash.addData(device.get())) {
        blockers->append(QStringLiteral("Unable to hash APFS target: %1")
                             .arg(device->errorString()));
        return {};
    }
...
bool imagePathIsSafeForCreate(const QString& imagePath, QStringList* blockers) {
    ...
    if (info.exists()) {
        blockers->append(QStringLiteral(
            "APFS image-only format refuses to overwrite an existing image path"));
        return false;
    }
    if (!info.dir().exists()) {
...
bool isCertifiedApfsObjectType(uint32_t objectType) {
    switch (objectType) {
    case kApfsObjectTypeNxSuperblock:
    case kApfsObjectTypeBtree:
    case kApfsObjectTypeBtreePhysical:
    case kApfsObjectTypeObjectMap:
    case kApfsObjectTypeFs:
    case kApfsObjectTypeSpaceman:
    case kApfsObjectTypeCheckpointMap:
    case kApfsObjectTypeReaper:
    case kApfsObjectTypeBtreeEphemeral:
    case kApfsObjectTypeChunkInfoBlock:
        return true;
    default:
        return false;
    }
...
bool readApfsRepairGeometry(QIODevice* image,
    ...
    if (le32(firstBlock, kApfsObjectMagicOffset) != kApfsMagicNxsb) {
        blockers->append(QStringLiteral("APFS repair requires a visible NXSB container superblock"));
        return false;
    }
...
bool readGeneratedLayoutBlock(QIODevice* image,
    ...
    if (blockIndex >= geometry.blockCount) {
        blockers->append(QStringLiteral(
            "APFS generated/minimal layout is missing required block %1").arg(blockIndex));
        return false;
    }
...
bool appendGeneratedHeaderBlockers(const GeneratedHeaderExpectation& expected) {
    ...
    appendGeneratedLayoutBlocker(le64(*expected.block, kApfsObjectOidOffset) ==
                                     expected.expectedOid,
                                 QStringLiteral("APFS generated/minimal block %1 OID mismatch")
                                     .arg(expected.blockIndex),
                                 expected.blockers);
    appendGeneratedLayoutBlocker(le64(*expected.block, kApfsObjectXidOffset) == kApfsFormatXid,
                                 QStringLiteral("APFS generated/minimal block %1 XID mismatch")
                                     .arg(expected.blockIndex),
                                 expected.blockers);
    appendGeneratedLayoutBlocker(le32(*expected.block, kApfsObjectTypeOffset) ==
                                     expected.expectedType,
                                 QStringLiteral("APFS generated/minimal block %1 type mismatch")
                                     .arg(expected.blockIndex),
                                 expected.blockers);
    appendGeneratedLayoutBlocker(le32(*expected.block, kApfsObjectSubtypeOffset) ==
                                     expected.expectedSubtype,
                                 QStringLiteral("APFS generated/minimal block %1 subtype mismatch")
                                     .arg(expected.blockIndex),
                                 expected.blockers);
    if (!PartitionApfsWriter::verifyObjectChecksum(*expected.block) &&
        ...
        expected.blockers->append(QStringLiteral(
            "APFS generated/minimal block %1 checksum is invalid").arg(expected.blockIndex));
    }
...
bool appendGeneratedObjectMapTreeBlockers(const GeneratedObjectMapTreeExpectation& expected) {
    ...
    appendGeneratedLayoutBlocker(
        ...
        QStringLiteral("APFS generated/minimal object-map tree block %1 flags mismatch")
            .arg(expected.blockIndex),
        expected.blockers);
    appendGeneratedLayoutBlocker(le32(*expected.block, kApfsBtreeNodeCountOffset) == 1,
                                 QStringLiteral(
                                     "APFS generated/minimal object-map tree block %1 key count mismatch")
                                     .arg(expected.blockIndex),
                                 expected.blockers);
    const qsizetype keyOffset = kApfsBtreeNodeHeaderBytes + 448;
    const qsizetype valueOffset =
        static_cast<qsizetype>(kSupportedApfsBlockSizeBytes) - kApfsBtreeInfoBytes -
        kApfsObjectMapValueBytes;
    appendGeneratedLayoutBlocker(le64(*expected.block, keyOffset) == expected.expectedMapOid,
                                 QStringLiteral(
                                     "APFS generated/minimal object-map tree block %1 map OID mismatch")
                                     .arg(expected.blockIndex),
                                 expected.blockers);
    appendGeneratedLayoutBlocker(le64(*expected.block, keyOffset + kApfsOmapKeyXidOffset) ==
                                     kApfsFormatXid,
                                 QStringLiteral(
                                     "APFS generated/minimal object-map tree block %1 map XID mismatch")
                                     .arg(expected.blockIndex),
                                 expected.blockers);
    appendGeneratedLayoutBlocker(le64(*expected.block, valueOffset + kApfsOmapValuePaddrOffset) ==
                                     expected.expectedPhysicalBlock,
                                 QStringLiteral(
                                     "APFS generated/minimal object-map tree block %1 physical block mismatch")
                                     .arg(expected.blockIndex),
                                 expected.blockers);
    return expected.blockers->size() == before;
}
...
bool appendGeneratedRootTreeBlockers(const GeneratedRootTreeExpectation& expected) {
    ...
    appendGeneratedLayoutBlocker((flags & (kApfsBtreeNodeRoot | kApfsBtreeNodeLeaf)) ==
                                 ...
                                 QStringLiteral(
                                     "APFS generated/minimal root tree flags mismatch"),
                                 expected.blockers);
    appendGeneratedLayoutBlocker(keys > 0 &&
                                     keys <= 1 + (kApfsWriteRootListingMaxEntries *
                                                  kApfsGeneratedRootRecordsPerFile),
                                 QStringLiteral(
                                     "APFS generated/minimal root tree key count is out of bounds"),
                                 expected.blockers);
    return expected.blockers->size() == before;
}
...
bool appendGeneratedGeometryBlocker(const GeneratedApfsLayoutContext& context) {
    if (context.geometry.blockSize != kSupportedApfsBlockSizeBytes ||
        ...
        context.blockers->append(QStringLiteral(
            "APFS %1 requires a S.A.K.-generated/minimal 4096-byte block layout")
                                     .arg(context.purpose));
        return false;
    }
...
bool readGeneratedApfsLayoutBlocks(const GeneratedApfsLayoutContext& context,
    ...
    return readGeneratedLayoutBlock(context.image,
                                    context.geometry,
                                    kApfsFormatNxsbBlock,
                                    &blocks->nxsb,
                                    context.blockers) &&
           readGeneratedLayoutBlock(context.image,
                                    context.geometry,
           ...
           readGeneratedLayoutBlock(context.image,
                                    ...
                                    &blocks->containerOmap,
                                    context.blockers) &&
           readGeneratedLayoutBlock(context.image,
                                    context.geometry,
                                    kApfsFormatContainerOmapTreeBlock,
                                    &blocks->containerOmapTree,
                                    context.blockers) &&
           readGeneratedLayoutBlock(context.image,
                                    context.geometry,
...
void appendGeneratedNxsbBlockers(const GeneratedApfsLayoutContext& context,
    ...
    appendGeneratedLayoutBlocker(le64(blocks.nxsb, kApfsNxOmapOidOffset) ==
                                 ...
                                 context.blockers);
    appendGeneratedLayoutBlocker(le32(blocks.nxsb, kApfsNxMaxFileSystemsOffset) == 1,
                                 QStringLiteral(
                                     "APFS generated/minimal max file systems mismatch"),
                                 context.blockers);
    appendGeneratedLayoutBlocker(le64(blocks.nxsb, kApfsNxFsOidArrayOffset) ==
                                     kApfsFormatVolumeOid,
...
void appendGeneratedCheckpointBlockers(const GeneratedApfsLayoutContext& context,
    ...
    appendGeneratedLayoutBlocker(
        ...
        context.blockers);
    appendGeneratedLayoutBlocker(
        le64(blocks.nxsb, kApfsNxReaperOidOffset) == kApfsFormatReaperOid,
        QStringLiteral("APFS generated/minimal reaper OID mismatch"),
        context.blockers);
    appendGeneratedLayoutBlocker(
        le64(blocks.nxsb, kApfsNxEphemeralInfoOffset) == kApfsNxEphemeralInfoValue,
    ...
    appendGeneratedLayoutBlocker(
        le32(blocks.checkpointMap, entry) == kApfsObjectTypeSpaceman &&
            le64(blocks.checkpointMap, entry + kApfsCheckpointMapEntryOidOffset) ==
                kApfsFormatSpacemanOid &&
            le64(blocks.checkpointMap, entry + kApfsCheckpointMapEntryPaddrOffset) ==
                kApfsFormatSpacemanBlock,
        QStringLiteral(
            "APFS generated/minimal checkpoint map must resolve the space manager"),
        context.blockers);
    const qsizetype reaperEntry = entry + kApfsCheckpointMapEntryBytes;
    appendGeneratedLayoutBlocker(
        le32(blocks.checkpointMap, reaperEntry) == kApfsObjectTypeReaper &&
            le64(blocks.checkpointMap, reaperEntry + kApfsCheckpointMapEntryOidOffset) ==
                kApfsFormatReaperOid &&
            le64(blocks.checkpointMap, reaperEntry + kApfsCheckpointMapEntryPaddrOffset) ==
                kApfsFormatReaperBlock,
    ...
    appendGeneratedLayoutBlocker(
        ...
        QStringLiteral(
            "APFS generated/minimal checkpoint superblock copy must match block zero"),
        context.blockers);
}
...
void appendGeneratedOmapSnapshotStateBlockers(const QByteArray& omap,
                                              QStringView label,
                                              QStringList* blockers) {
    appendGeneratedLayoutBlocker(le32(omap, kApfsOmapSnapshotCountOffset) == 0 &&
                                     le64(omap, kApfsOmapSnapshotTreeOidOffset) == 0 &&
                                     le64(omap, kApfsOmapMostRecentSnapshotOffset) == 0,
                                 QStringLiteral(
                                     "APFS generated/minimal %1 object map must not carry snapshot state")
                                     .arg(label),
                                 blockers);
    appendGeneratedLayoutBlocker(le64(omap, kApfsOmapPendingRevertMinOffset) == 0 &&
                                     le64(omap, kApfsOmapPendingRevertMaxOffset) == 0,
                                 QStringLiteral(
                                     "APFS generated/minimal %1 object map must not carry pending revert state")
                                     .arg(label),
                                 blockers);
}
...
void appendGeneratedContainerMapBlockers(const GeneratedApfsLayoutContext& context,
    ...
    appendGeneratedHeaderBlockers({.block = &blocks.containerOmap,
                                   ...
                                   .allowChecksumRepair = context.allowChecksumRepair,
                                   .blockers = context.blockers});
    appendGeneratedLayoutBlocker(le64(blocks.containerOmap, kApfsOmapTreeOidOffset) ==
                                     kApfsFormatContainerOmapTreeBlock,
                                 QStringLiteral(
                                     "APFS generated/minimal container object-map tree mismatch"),
                                 context.blockers);
...
void appendGeneratedVolumeBlockers(const GeneratedApfsLayoutContext& context,
    ...
    appendGeneratedLayoutBlocker(le32(blocks.volume, kApfsObjectMagicOffset) == kApfsMagicApsb,
                                 ...
                                 context.blockers);
    appendGeneratedLayoutBlocker(le64(blocks.volume, kApfsVolumeOmapOidOffset) ==
                                     kApfsFormatVolumeOmapBlock,
                                 QStringLiteral(
                                     "APFS generated/minimal volume object-map OID mismatch"),
                                 context.blockers);
    appendGeneratedLayoutBlocker(le64(blocks.volume, kApfsVolumeRootTreeOidOffset) ==
                                 ...
                                 context.blockers);
    appendGeneratedLayoutBlocker(le64(blocks.volume, kApfsVolumeSnapMetaTreeOidOffset) ==
                                     kApfsFormatSnapMetaTreeBlock,
                                 QStringLiteral(
                                     "APFS generated/minimal snapshot-metadata tree OID mismatch"),
                                 context.blockers);
    appendGeneratedLayoutBlocker(le64(blocks.volume, 0x90) == kApfsFormatExtentRefTreeBlock,
                                 ...
                                 context.blockers);
    appendGeneratedLayoutBlocker(le64(blocks.volume, kApfsVolumeRevertToXidOffset) == 0 &&
                                     le64(blocks.volume, kApfsVolumeRevertToSblockOidOffset) == 0,
                                 QStringLiteral(
                                     "APFS generated/minimal volume must not carry revert metadata"),
                                 context.blockers);
    appendGeneratedLayoutBlocker(le64(blocks.volume, kApfsVolumeNumSnapshotsOffset) == 0,
                                 ...
                                 context.blockers);
    appendGeneratedLayoutBlocker(le64(blocks.volume, kApfsVolumeFsFlagsOffset) == 1,
                                 QStringLiteral(
                                     "APFS generated/minimal volume flags must match the generated layout; "
    ...
    appendGeneratedHeaderBlockers({.block = &blocks.volumeOmap,
                                   ...
                                   .allowChecksumRepair = context.allowChecksumRepair,
                                   .blockers = context.blockers});
    appendGeneratedLayoutBlocker(le64(blocks.volumeOmap, kApfsOmapTreeOidOffset) ==
                                     kApfsFormatVolumeOmapTreeBlock,
                                 QStringLiteral(
                                     "APFS generated/minimal volume object-map tree mismatch"),
                                 context.blockers);
...
void appendGeneratedSpacemanBlockers(const GeneratedApfsLayoutContext& context,
    ...
    appendGeneratedHeaderBlockers({.block = &blocks.spaceman,
                                   ...
                                   .allowChecksumRepair = context.allowChecksumRepair,
                                   .blockers = context.blockers});
    appendGeneratedLayoutBlocker(le32(blocks.spaceman, kApfsSpacemanBlockSizeOffset) ==
                                     context.geometry.blockSize,
                                 QStringLiteral(
                                     "APFS generated/minimal spaceman block size mismatch"),
                                 context.blockers);
    const uint64_t allocatedBlocks =
        le64(blocks.volume, kApfsVolumeAllocatedBlockCountOffset);
    const uint64_t freeBlocks =
        le64(blocks.spaceman,
             kApfsSpacemanMainDeviceOffset + kApfsSpacemanDeviceFreeCountOffset);
    ...
    appendGeneratedLayoutBlocker(chunkCount == 1 && cibCount == 1 && chunkInfoCount == 1,
                                 ...
                                 context.blockers);
    appendGeneratedLayoutBlocker(chunkInfoBlocks == mainBlocks &&
                                     chunkInfoBlocks <= kGeneratedApfsSingleChunkMaxBlocks,
                                 QStringLiteral(
                                     "APFS generated/minimal chunk-info block count exceeds "
                                     "the certified one-chunk layout"),
                                 context.blockers);
    ...
    const uint64_t chunkFreeBlocks =
        le32(blocks.chunkInfo,
             kApfsChunkInfoEntriesOffset + kApfsChunkInfoEntryFreeCountOffset);
    appendGeneratedLayoutBlocker(
...
bool repairApfsObjectChecksumBlock(const ApfsRepairBlockContext& context,
                                   ...
                                   QStringList* blockers) {
    *repaired = false;
    if (!readApfsRepairBlock(context.image,
                             context.geometry,
                             context.blockIndex,
                             context.block,
                             blockers)) {
    ...
    *repaired = true;
    return writeApfsRepairBlock(context.image,
                                context.geometry,
                                context.blockIndex,
                                *context.block,
                                blockers);
...
bool repairApfsObjectChecksumBlocks(QIODevice* image,
    ...
    *counters = {};
    ...
    for (uint64_t blockIndex = 0; blockIndex < geometry.blockCount; ++blockIndex) {
        bool repaired = false;
        if (!repairApfsObjectChecksumBlock({.image = image,
                                            .geometry = geometry,
...
bool repairGeneratedApfsObjectChecksumBlocks(QIODevice* image,
    ...
    *counters = {};
    QByteArray block(static_cast<qsizetype>(geometry.blockSize), '\0');
    const QVector<uint64_t> generatedMetadataBlocks{
        kApfsFormatNxsbBlock,
        kApfsFormatCheckpointMapBlock,
        kApfsFormatCheckpointNxsbCopyBlock,
        kApfsFormatContainerOmapBlock,
...
constexpr std::array<ApfsEvidenceDescription, 11> kEvidenceDescriptions{{
    ...
    {kEvidenceObjectChecksumVectors,
     QLatin1StringView("APFS object checksum evidence is missing")},
    ...
    {kEvidenceRollbackBoundary,
     QLatin1StringView("APFS rollback-boundary evidence is missing")},
}};
...
[[nodiscard]] QStringList planRequiredEvidenceIds(
...
constexpr std::array<ApfsEvidenceFlag, 11> kEvidenceFlags{{
    {kEvidenceStructureMapping,
     &PartitionApfsWriterExecutionEvidence::structure_mapping_verified},
    {kEvidenceObjectChecksumVectors,
     &PartitionApfsWriterExecutionEvidence::object_checksum_vectors_verified},
    {kEvidenceSourceImageHash,
     &PartitionApfsWriterExecutionEvidence::source_image_hash_verified},
    {kEvidenceScratchImageHash,
     &PartitionApfsWriterExecutionEvidence::scratch_image_hash_verified},
    {kEvidenceCopyOnWriteCheckpoint,
     &PartitionApfsWriterExecutionEvidence::copy_on_write_checkpoint_verified},
    {kEvidenceObjectMapUpdate,
     &PartitionApfsWriterExecutionEvidence::object_map_update_verified},
    {kEvidenceSpaceManagerAccounting,
     &PartitionApfsWriterExecutionEvidence::space_manager_accounting_verified},
    {kEvidenceFsckValidation,
     &PartitionApfsWriterExecutionEvidence::fsck_validation_verified},
    {kEvidenceTargetReadback,
     &PartitionApfsWriterExecutionEvidence::target_readback_verified},
    {kEvidenceCrashReplay,
     &PartitionApfsWriterExecutionEvidence::crash_replay_verified},
    {kEvidenceRollbackBoundary,
     &PartitionApfsWriterExecutionEvidence::rollback_boundary_verified},
}};

[[nodiscard]] QStringList verifiedEvidenceIds(const PartitionApfsWriterExecutionEvidence& evidence) {
...
void appendAlwaysRequiredApfsWriterBlockers(const PartitionApfsWriteOptions& options,
    ...
    if (options.evidence_id.trimmed().isEmpty()) {
        result->blockers.append(QStringLiteral(
            "APFS mutation requires a linked certification evidence ID"));
    }
}
...
void appendDetectionBlockers(const PartitionFileSystemDetection& detection,
    ...
    if (hasDetailContaining(detection.details, QStringLiteral("Metadata sanity warning:"))) {
        result->blockers.append(QStringLiteral(
            "APFS metadata sanity warnings must be resolved before any mutation"));
    }
    if (detection.total_bytes == 0) {
        result->blockers.append(QStringLiteral(
            "APFS writer requires known container size metadata"));
    }
}
...
void appendFeatureBlockers(const PartitionFileSystemDetection& detection,
    ...
    const auto incompatible =
        parseHexDetail(detection.details, QStringLiteral("Incompatible features:"));
    const uint64_t incompatibleMask = incompatible.value_or(0);
    const uint64_t unsupportedIncompatible =
        incompatibleMask & ~static_cast<uint64_t>(kApfsContainerIncompatVersion2);
    if (unsupportedIncompatible != 0 && !options.allow_encrypted_or_protected_volume) {
        result->blockers.append(QStringLiteral(
            "APFS unsupported incompatible feature flags are present; protected/encrypted variants stay blocked"));
    }
    ...
    if (!options.allow_multi_volume_container) {
        const auto volumeSlots =
            parseDecimalDetail(detection.details, QStringLiteral("Volume OID slots used:"));
...
void appendApfsVolumeNameBlockers(const QString& volumeName,
    ...
    if (volumeName.size() > kMaximumApfsVolumeNameChars) {
        blockers->append(QStringLiteral("APFS %1 volume name exceeds 255 characters")
                             .arg(purpose));
    }
    if (volumeName.toUtf8().size() >= kApfsVolumeNameBytes) {
        blockers->append(QStringLiteral("APFS %1 volume name exceeds 255 UTF-8 bytes")
                             .arg(purpose));
    }
...
void appendOperationBlockers(PartitionApfsWriteOperation operation,
    ...
    if (apfsOperationRequiresPayloadCap(operation) && options.max_payload_bytes == 0) {
        result->blockers.append(QStringLiteral(
            "APFS file/directory mutation requires a bounded payload size"));
    }
    if (options.max_payload_bytes > kDefaultMaxApfsPayloadBytes) {
        result->blockers.append(QStringLiteral(
            "APFS payload exceeds current certification cap"));
    }
...
PartitionApfsWritePreflight preflightNewContainerFormat(uint64_t target_container_bytes,
    ...
    result.warnings.append(QStringLiteral(
        "APFS format is limited to S.A.K. generated single-volume containers with 4096-byte blocks"));
    ...
    if (block_size_bytes != kSupportedApfsBlockSizeBytes) {
        result.blockers.append(QStringLiteral(
            "APFS format certification currently supports only 4096-byte blocks"));
    }
...
void appendPlanTargetBlockers(PartitionApfsWriteOperation operation,
    ...
    if (normalized_path.isEmpty()) {
        plan->preflight.blockers.append(QStringLiteral(
            "APFS file/directory mutation requires an absolute target path"));
        return;
    }
    if (containsUnsafePathSegment(normalized_path)) {
        plan->preflight.blockers.append(QStringLiteral(
            "APFS mutation target path contains traversal segments"));
    }
}
...
QVector<PartitionApfsImageMutationStep> fileMutationSteps(PartitionApfsWriteOperation operation) {
    ...
    steps.append({QStringLiteral("preflight"),
                  QStringLiteral(
                      "Verify read-only APFS metadata, feature flags, object checksums, and linked certification evidence"),
                  false,
                  false,
                  {QString(kEvidenceStructureMapping), QString(kEvidenceObjectChecksumVectors)}});
    steps.append({QStringLiteral("scratch-image"),
                  QStringLiteral(
    ...
    steps.append({QStringLiteral("block-allocation"),
                  QStringLiteral(
                      "Reserve new image blocks from the APFS free-space model without overwriting existing objects"),
                  true,
                  true,
                  {QString(kEvidenceSpaceManagerAccounting)}});
    steps.append({QStringLiteral("fs-tree-update"),
                  QStringLiteral(
    ...
    if (operation == PartitionApfsWriteOperation::ReplaceFile ||
        ...
        steps.append({QStringLiteral("extent-reference-update"),
                      QStringLiteral(
                          "Update extent-reference accounting for replaced or deleted file data"),
                      true,
                      true,
                      {QString(kEvidenceStructureMapping),
                       QString(kEvidenceSpaceManagerAccounting),
    ...
    steps.append({QStringLiteral("object-map-update"),
                  QStringLiteral(
                      "Write updated object-map records that point each mutated object ID at a new physical block"),
                  true,
                  true,
                  {QString(kEvidenceObjectMapUpdate),
                   QString(kEvidenceObjectChecksumVectors),
    ...
    steps.append({QStringLiteral("checkpoint-commit"),
                  QStringLiteral(
                      "Write a new checkpoint map and container superblock checkpoint with monotonic transaction ID"),
                  true,
                  true,
                  {QString(kEvidenceCopyOnWriteCheckpoint),
                   QString(kEvidenceRollbackBoundary),
    ...
    steps.append({QStringLiteral("post-verify"),
                  QStringLiteral(
                      "Reopen the scratch image read-only, run platform APFS verification, and compare targeted read-back"),
                  false,
                  false,
                  {QString(kEvidenceFsckValidation),
                   QString(kEvidenceTargetReadback),
...
QVector<PartitionApfsImageMutationStep> formatContainerSteps() {
    ...
    steps.append({QStringLiteral("target-image-snapshot"),
                  QStringLiteral(
                      "Create a scratch target image, hash any pre-format bytes, and keep rollback artifacts"),
                  false,
                  false,
                  {QString(kEvidenceSourceImageHash), QString(kEvidenceScratchImageHash)}});
    steps.append({QStringLiteral("container-layout"),
                  QStringLiteral(
                      "Lay out APFS NXSB container, object header fields, block geometry, feature flags, and UUIDs"),
                  true,
                  true,
                  {QString(kEvidenceStructureMapping), QString(kEvidenceObjectChecksumVectors)}});
    steps.append({QStringLiteral("checkpoint-regions"),
                  QStringLiteral(
                  ...
                  {QString(kEvidenceCopyOnWriteCheckpoint),
                   QString(kEvidenceRollbackBoundary),
                   QString(kEvidenceObjectChecksumVectors)}});
    steps.append({QStringLiteral("space-manager-init"),
                  QStringLiteral(
                      "Initialize spaceman counters, device records, free-space accounting, and allocation state"),
                  true,
                  true,
                  {QString(kEvidenceSpaceManagerAccounting),
                   QString(kEvidenceObjectChecksumVectors)}});
    ...
    steps.append({QStringLiteral("volume-superblock-root-tree-init"),
                  QStringLiteral(
                      "Create APFS volume superblock, empty root directory tree, extent tree, and metadata tree objects"),
                  true,
                  true,
                  {QString(kEvidenceStructureMapping),
                   QString(kEvidenceObjectChecksumVectors),
    ...
    steps.append({QStringLiteral("post-verify"),
                  QStringLiteral(
                      "Verify the formatted scratch image with platform APFS tooling and S.A.K. read-only browse"),
                  false,
                  false,
                  {QString(kEvidenceFsckValidation),
                   QString(kEvidenceTargetReadback),
...
QVector<PartitionApfsImageMutationStep> repairContainerSteps() {
    ...
    steps.append({QStringLiteral("repair-scan"),
                  QStringLiteral(
                      "Scan all reachable checkpoints, object maps, spaceman objects, volume superblocks, and B-tree roots read-only"),
                  false,
                  false,
                  {QString(kEvidenceStructureMapping), QString(kEvidenceObjectChecksumVectors)}});
    steps.append({QStringLiteral("scratch-image"),
    ...
    steps.append({QStringLiteral("object-map-rebuild-plan"),
                  QStringLiteral(
                      "Build a repair plan that maps each surviving object ID to a verified replacement block"),
                  true,
                  true,
                  {QString(kEvidenceObjectMapUpdate),
                   QString(kEvidenceStructureMapping),
                   QString(kEvidenceCopyOnWriteCheckpoint)}});
    steps.append({QStringLiteral("space-manager-reconcile"),
                  QStringLiteral(
                  ...
                  {QString(kEvidenceSpaceManagerAccounting),
                   QString(kEvidenceObjectChecksumVectors)}});
    steps.append({QStringLiteral("repaired-checkpoint-commit"),
                  QStringLiteral(
    ...
    steps.append({QStringLiteral("post-verify"),
                  QStringLiteral(
                      "Verify repaired scratch image with fsck_apfs/diskutil, S.A.K. browse, and crash replay"),
                  false,
                  false,
                  {QString(kEvidenceFsckValidation),
                   QString(kEvidenceTargetReadback),
...
QVector<PartitionApfsImageMutationStep> resizeContainerSteps() {
    QVector<PartitionApfsImageMutationStep> steps;
    steps.append({QStringLiteral("resize-preflight"),
                  QStringLiteral(
    ...
    steps.append({QStringLiteral("scratch-image"),
                  ...
                  false,
                  {QString(kEvidenceSourceImageHash), QString(kEvidenceScratchImageHash)}});
    steps.append({QStringLiteral("container-size-update"),
                  QStringLiteral(
                  ...
                  {QString(kEvidenceStructureMapping),
                   QString(kEvidenceCopyOnWriteCheckpoint),
                   QString(kEvidenceObjectChecksumVectors)}});
    steps.append({QStringLiteral("space-manager-rebalance"),
                  QStringLiteral(
    ...
    steps.append({QStringLiteral("post-verify"),
                  QStringLiteral(
                      "Verify resized scratch image with platform APFS tooling, read-back, and crash replay"),
                  false,
                  false,
                  {QString(kEvidenceFsckValidation),
                   QString(kEvidenceTargetReadback),
...
QVector<PartitionApfsImageMutationStep> volumeLabelChangeSteps() {
    ...
    steps.append({QStringLiteral("preflight-generated-layout"),
                  QStringLiteral(
                      "Verify S.A.K.-generated APFS layout, block geometry, and object checksums before label mutation"),
                  false,
                  false,
                  {QString(kEvidenceStructureMapping), QString(kEvidenceObjectChecksumVectors)}});
    steps.append({QStringLiteral("scratch-or-selected-target"),
                  QStringLiteral(
                      "Clone image output or require selected raw partition confirmation before metadata write"),
                  false,
                  false,
                  {QString(kEvidenceSourceImageHash), QString(kEvidenceScratchImageHash)}});
    steps.append({QStringLiteral("volume-superblock-label-update"),
                  QStringLiteral(
                      "Update APFS volume superblock name field and restamp object checksum"),
                  true,
                  true,
                  {QString(kEvidenceStructureMapping),
                   QString(kEvidenceObjectChecksumVectors),
    ...
    steps.append({QStringLiteral("post-verify"),
                  QStringLiteral(
                      "Reopen target read-only and verify S.A.K. APFS browser reports the requested volume name"),
                  false,
                  false,
...
void appendMissingExecutionEvidenceBlockers(
    const PartitionApfsImageMutationPlan& plan,
    ...
    for (const auto& artifact : evidence.artifacts) {
        if (isUnsafeEvidenceArtifactPath(normalizedEvidenceArtifactPath(artifact))) {
            result->blockers.append(QStringLiteral(
                "APFS execution evidence artifact path must be relative under artifacts/ and traversal-free"));
            break;
        }
    }
}
...
void appendPlanEvidenceMismatchBlockers(const PartitionApfsImageMutationPlan& plan,
    ...
    if (plan.operation != evidence.operation.trimmed()) {
        result->blockers.append(QStringLiteral("APFS execution evidence operation does not match plan"));
    }
    if (plan.target_path != normalizedApfsTargetPath(evidence.target_path)) {
        result->blockers.append(QStringLiteral("APFS execution evidence target path does not match plan"));
    }
}

QStringList imageOnlyExecutionBlockers() {
    return {QStringLiteral("APFS image mutation execution is blocked until structured evidence is supplied"),
            QStringLiteral("Raw APFS media exposure remains blocked until separate hardware proof is supplied")};
}
...
bool PartitionApfsWriter::stampObjectChecksum(QByteArray* object_bytes) {
    ...
    qToLittleEndian<uint64_t>(*checksum,
                              reinterpret_cast<uchar*>(object_bytes->data()));
    return true;
}
...
PartitionApfsWritePreflight PartitionApfsWriter::preflightExistingContainer(
    ...
    result.warnings.append(QStringLiteral(
        "APFS mutation remains limited to certified generated/minimal layouts; arbitrary Apple APFS mutation stays blocked"));
...
PartitionApfsImageMutationPlan PartitionApfsWriter::planImageOnlyMutation(
    ...
    if (const auto blockSize =
            parseDecimalDetail(detection.details, QStringLiteral("Block size:"))) {
    ...
    if (operation == PartitionApfsWriteOperation::ChangeVolumeLabel) {
        plan.post_apply_verification = {
            ...
            QStringLiteral("Verify source-image and scratch-image hash chain before and after mutation"),
...
PartitionApfsImageMutationPlan PartitionApfsWriter::planImageOnlyFormat(
    ...
    plan.preflight = preflightNewContainerFormat(target_container_bytes,
                                                 block_size_bytes,
                                                 plan.volume_name,
                                                 options);
    ...
    plan.post_apply_verification = {
        ...
        QStringLiteral("Run fsck_apfs/diskutil read-only verification on the formatted scratch image"),
        ...
        QStringLiteral("Record crash-interruption replay artifacts for format checkpoint publication")};
    return plan;
}
...
PartitionApfsImageBuildResult formatBuildResult(const PartitionApfsImageFormatRequest& request) {
    ...
    result.warnings.append(request.options.image_only
                               ? QStringLiteral(
                                     "Generated APFS image format remains image-only; raw partition format is exposed only through sak_apfs_writer_cli.exe with explicit selected-target confirmation")
                               : QStringLiteral(
                                     "Raw APFS format is limited to selected partitions through the APFS writer helper with explicit confirmation"));
    return result;
}
...
bool appendFormatGeometryBlockers(const PartitionApfsImageFormatRequest& request,
    ...
    if (request.target_container_bytes % request.block_size_bytes != 0) {
        result->blockers.append(QStringLiteral(
            "APFS format target size must be an exact multiple of block size"));
        return false;
    }
...
bool appendRawFormatTargetBlockers(const PartitionApfsImageFormatRequest& request,
    ...
    if (!request.allow_raw_device_target) {
        result->blockers.append(QStringLiteral(
            "APFS raw-device format target requires explicit raw-device opt-in"));
        return false;
    }
    if (request.options.image_only) {
        result->blockers.append(QStringLiteral(
            "APFS raw-device format target requires non-image-only writer options"));
        return false;
    }
    return true;
}
...
bool appendFormatWipeConfirmationBlocker(const PartitionApfsImageFormatRequest& request,
    ...
    if (!request.target_wipe_confirmed) {
        result->blockers.append(QStringLiteral(
            "APFS existing-image format requires destructive wipe confirmation"));
        return false;
    }
    return true;
}
...
bool createSizedApfsImage(const QString& path,
    ...
    if (!image->open(QIODevice::WriteOnly)) {
        blockers->append(QStringLiteral("Unable to create APFS image: %1")
                             .arg(image->errorString()));
        return false;
    }
    if (!image->resize(static_cast<qint64>(sizeBytes))) {
        blockers->append(QStringLiteral("Unable to size APFS image: %1")
                             .arg(image->errorString()));
        return false;
    }
    return true;
...
QVector<ApfsImageBlock> emptyFormatBlocks(const PartitionApfsImageFormatRequest& request,
    ...
    const QVector<ApfsObjectMapEntry> containerMappings{
        {kApfsFormatVolumeOid, kApfsFormatXid, kApfsFormatVolumeSuperblockBlock}};
    const QVector<ApfsObjectMapEntry> volumeMappings{
        {kApfsFormatRootTreeOid, kApfsFormatXid, kApfsFormatRootTreeBlock}};
    const QByteArray nxsb = buildNxSuperblock(request.block_size_bytes,
                                              blockCount,
                                              containerUuid,
                                              {},
                                              blockers);
    const QByteArray genesisNxsb =
        buildNxSuperblock(request.block_size_bytes,
                          blockCount,
                          containerUuid,
                          {.xid = kApfsFormatGenesisXid,
                           ...
                           .dataLen = 2,
                           .dataNext = 2,
                           .omapOid = kApfsFormatGhostContainerOmapBlock,
                           .fsOid = 0,
    ...
    const QVector<ApfsCheckpointMapEntry> liveMappings{
        ...
        {kApfsObjectTypeBtreeEphemeral, kApfsObjectSubtypeFreeQueue,
         kApfsFormatFqIpTreeOid, kApfsFormatFqIpTreeBlock},
        {kApfsObjectTypeBtreeEphemeral, kApfsObjectSubtypeFreeQueue,
         kApfsFormatFqMainTreeOid, kApfsFormatFqMainTreeBlock}};
    return {{kApfsFormatNxsbBlock, nxsb},
            ...
            {kApfsFormatGenesisSpacemanBlock,
             buildSpacemanBlock(request.block_size_bytes,
                                blockCount,
                                kApfsFormatGhostReservedBlocks,
                                kApfsFormatGenesisXid,
                                true,
                                blockers)},
            {kApfsFormatGenesisReaperBlock,
            ...
            {kApfsFormatFqMainTreeBlock,
             buildFreeQueueTreeBlock(request.block_size_bytes,
                                     ...
                                     2,
                                     blockers)},
            {kApfsFormatIpBitmapBaseBlock,
             buildIpBitmapBlock(request.block_size_bytes, 0x3)},
            {kApfsFormatIpBitmapBaseBlock + 1,
             buildIpBitmapBlock(request.block_size_bytes, 0xF)},
            {kApfsFormatGhostChunkInfoBlock,
             buildChunkInfoBlock(request.block_size_bytes,
                                 blockCount,
                                 kApfsFormatGhostReservedBlocks,
                                 kApfsFormatGenesisXid,
                                 kApfsFormatGhostChunkInfoBlock,
                                 ...
                                 blockers)},
            {kApfsFormatGhostChunkBitmapBlock,
             buildChunkBitmapBlock(request.block_size_bytes, kApfsFormatGhostReservedBlocks)},
            {kApfsFormatChunkInfoBlock,
             buildChunkInfoBlock(request.block_size_bytes,
                                 blockCount,
                                 kApfsFormatSeedFileDataBlock,
                                 ...
                                 blockers)},
            {kApfsFormatChunkBitmapBlock,
             buildChunkBitmapBlock(request.block_size_bytes, kApfsFormatSeedFileDataBlock)},
            {kApfsFormatGhostContainerOmapBlock,
             buildObjectMapBlock(request.block_size_bytes,
                                 kApfsFormatGhostContainerOmapBlock,
                                 kApfsFormatGhostContainerOmapTreeBlock,
                                 kApfsFormatGenesisXid,
                                 1,
            ...
            {kApfsFormatSnapMetaTreeBlock,
             buildEmptyVariableTreeBlock(request.block_size_bytes,
                                         ...
                                         blockers)},
            {kApfsFormatRootTreeBlock,
             buildEmptyRootTreeBlock(request.block_size_bytes, blockers)},
            {kApfsFormatVolumeSuperblockBlock,
             buildVolumeSuperblock({.blockSize = request.block_size_bytes,
                                    .volumeName = volumeName,
                                    .volumeUuid = volumeUuid,
                                    .allocatedBlocks =
                                        kApfsFormatVolumeBaseAllocatedBlocks},
                                   blockers)},
            {kApfsFormatContainerOmapBlock,
             buildObjectMapBlock(request.block_size_bytes,
                                 kApfsFormatContainerOmapOid,
...
bool writeZeroRange(QIODevice* device,
                    uint64_t offset,
                    uint64_t length,
                    QStringList* blockers) {
    ...
    if (!device->seek(static_cast<qint64>(offset))) {
        blockers->append(QStringLiteral("Unable to seek APFS zero-fill target: %1")
                             .arg(device->errorString()));
        return false;
    }
    ...
    while (remaining > 0) {
        const qsizetype chunk =
            static_cast<qsizetype>(std::min<uint64_t>(remaining,
                                                      static_cast<uint64_t>(zeroChunk.size())));
        if (device->write(zeroChunk.constData(), chunk) != chunk) {
            blockers->append(QStringLiteral("Unable to zero APFS format target: %1")
                                 .arg(device->errorString()));
            return false;
        }
...
bool appendSeedFileBlockers(const PartitionApfsImageFormatRequest& request,
    ...
    if (static_cast<uint64_t>(request.seed_file_data.size()) > kApfsMaximumSeedFileBytes) {
        result->blockers.append(QStringLiteral(
            "APFS seed-file data exceeds current image-only certification cap"));
    }
    result->ok = result->blockers.isEmpty();
    return result->ok;
}
...
uint64_t allocatedBlocksForFiles(const QVector<ApfsRootFilePayload>& files,
                                 uint32_t blockSize) {
    uint64_t allocatedBlocks = kApfsFormatSeedFileDataBlock;
    for (const auto& file : files) {
        allocatedBlocks = std::max(allocatedBlocks,
                                   file.dataStartBlock +
                                       roundedBlockCount(static_cast<uint64_t>(file.data.size()),
                                                         blockSize));
    }
...
QVector<ApfsRootDirectoryPayload> assignedRootDirectoryPayloads(
    const QVector<ApfsRootDirectoryInput>& inputs,
    qsizetype fileCount) {
    ...
    for (qsizetype index = 0; index < inputs.size(); ++index) {
        ...
        const uint64_t objectId =
            kApfsSeedFileId + static_cast<uint64_t>(fileCount) + static_cast<uint64_t>(index);
        directories.append({.directoryName = input.directoryName,
...
uint64_t parentDirectoryIdForFile(const ApfsRootFileInput& input,
    ...
    blockers->append(QStringLiteral(
        "APFS generated file parent directory was not preserved: %1")
...
QVector<ApfsRootFilePayload> assignedRootFilePayloads(
    ...
    for (qsizetype index = 0; index < inputs.size(); ++index) {
        const auto& input = inputs.at(index);
        const uint64_t parentDirectoryId =
            parentDirectoryIdForFile(input, directories, blockers);
...
QVector<ApfsImageBlock> seedRewriteBlocks(const ApfsSeedRewrite& rewrite,
                                          QStringList* blockers) {
    ...
    QVector<ApfsImageBlock> blocks{
        {kApfsFormatVolumeSuperblockBlock,
         buildVolumeSuperblock({.blockSize = rewrite.blockSize,
                                ...
                                .nextObjectId =
                                    kApfsFirstUserObjectId +
                                    static_cast<uint64_t>(rewrite.files.size()) +
        ...
        {kApfsFormatSpacemanBlock,
         buildSpacemanBlock(rewrite.blockSize,
                            rewrite.blockCount,
                             allocatedBlocks,
                             kApfsFormatXid,
                             false,
                             blockers)},
        {kApfsFormatChunkInfoBlock,
         buildChunkInfoBlock(rewrite.blockSize,
                             rewrite.blockCount,
                             allocatedBlocks,
                             kApfsFormatXid,
                             kApfsFormatChunkInfoBlock,
                             ...
                             blockers)},
        {kApfsFormatChunkBitmapBlock,
         buildChunkBitmapBlock(rewrite.blockSize, allocatedBlocks)}};
...
ApfsImageSource validateImageOnlySource(const QString& path,
    ...
    if (static_cast<uint64_t>(source.info.size()) > kDefaultMaxApfsPayloadBytes) {
        blockers->append(QStringLiteral(
            "APFS %1 source image exceeds current image-only certification cap").arg(purpose));
        return source;
    }
    source.ok = true;
    return source;
...
bool appendSeparateOutputBlockers(const QFileInfo& sourceInfo,
    ...
    if (QFileInfo(outputPath).absoluteFilePath() == sourceInfo.absoluteFilePath()) {
        blockers->append(QStringLiteral("APFS %1 output path must be separate from the source image")
                             .arg(purpose));
        return false;
    }
    return true;
...
bool runChecksumRepairOnScratch(QFile* image,
    ...
    ApfsRepairCounters counters;
    repairGeneratedApfsObjectChecksumBlocks(image,
                                            {.blockSize = blockSize, .blockCount = blockCount},
                                            &counters,
                                            blockers);
...
bool appendFileWritePayloadBlockers(const PartitionApfsImageRootFileWriteRequest& request,
    ...
    if (static_cast<uint64_t>(request.file_data.size()) > kApfsMaximumSeedFileBytes) {
        blockers->append(QStringLiteral(
            "APFS file-write data exceeds current image-only certification cap"));
    }
    if (request.options.max_payload_bytes > 0 &&
        static_cast<uint64_t>(request.file_data.size()) > request.options.max_payload_bytes) {
        blockers->append(QStringLiteral(
            "APFS file-write data exceeds configured payload bound"));
    }
    return blockers->isEmpty();
}
...
bool appendFilePatchPayloadBlockers(const PartitionApfsImageRootFilePatchRequest& request,
    ...
    if (static_cast<uint64_t>(request.patch_data.size()) > kApfsMaximumSeedFileBytes) {
        blockers->append(QStringLiteral(
            "APFS file-patch data exceeds current image-only certification cap"));
    }
    if (request.options.max_payload_bytes > 0 &&
        static_cast<uint64_t>(request.patch_data.size()) > request.options.max_payload_bytes) {
        blockers->append(QStringLiteral(
            "APFS file-patch data exceeds configured payload bound"));
    }
    return blockers->isEmpty();
}

PartitionApfsFileReadResult rootListingForWrite(const QString& imagePath,
                                                QStringList* blockers) {
    auto listing = PartitionApfsFileSystemReader::listDirectoryFromImage(
        imagePath,
        QStringLiteral("/"),
        kApfsWriteRootListingMaxEntries);
    if (!listing.ok) {
...
bool appendGeneratedSourceLayoutBlockers(const QString& sourcePath,
    ...
    return appendGeneratedApfsLayoutBlockers(source.get(),
                                             {.blockSize = blockSize, .blockCount = blockCount},
                                             purpose,
                                             false,
                                             blockers);
...
std::optional<PartitionApfsFileEntry> boundedRootFileEntry(
    ...
    for (const auto& entry : rootListing.entries) {
        ...
        if (!entry.regular_file || entry.size_bytes > kApfsMaximumSeedFileBytes) {
            blockers->append(QStringLiteral(
                "APFS %1 certification can mutate only bounded root regular files")
                                 .arg(purpose));
            return std::nullopt;
        }
        return entry;
...
std::optional<QByteArray> readBoundedRootFile(const QString& imagePath,
    ...
    if (!readBack.ok || static_cast<uint64_t>(readBack.data.size()) != entry.size_bytes) {
        blockers->append(QStringLiteral("APFS %1 could not read target root file %2")
                             .arg(purpose, entry.name));
        return std::nullopt;
    }
...
bool appendEmptyRootDirectoryInput(const QString& imagePath,
    ...
    if (!listing.ok) {
        blockers->append(QStringLiteral("APFS directory mutation could not verify root directory %1: %2")
...
bool appendExistingRootEntries(const QString& imagePath,
    ...
    for (const auto& entry : rootListing.entries) {
        ...
        if (entry.directory) {
            if (!appendEmptyRootDirectoryInput(imagePath,
                                               entry,
        ...
        if (!entry.regular_file || entry.size_bytes > kApfsMaximumSeedFileBytes) {
            blockers->append(QStringLiteral(
                "APFS generated-layout mutation can preserve only bounded root regular files and empty root directories"));
            return false;
        }
        const auto readBack = PartitionApfsFileSystemReader::readFileFromImage(
            imagePath,
            ...
            kApfsMaximumSeedFileBytes);
        if (!readBack.ok || static_cast<uint64_t>(readBack.data.size()) != entry.size_bytes) {
            blockers->append(QStringLiteral("APFS file-write could not preserve existing root file %1")
                                 .arg(entry.name));
            return false;
        }
...
bool appendBoundedRegularFileInput(const PartitionApfsFileEntry& entry,
    ...
    if (!entry.regular_file || entry.size_bytes > kApfsMaximumSeedFileBytes) {
        context.blockers->append(QStringLiteral(
            "APFS %1 can preserve only bounded regular files in generated layouts")
                                      .arg(context.purpose));
        return false;
    }
    const auto readBack =
        PartitionApfsFileSystemReader::readFileFromImage(*context.imagePath,
                                                         entry.path,
                                                         kApfsMaximumSeedFileBytes);
    if (!readBack.ok || static_cast<uint64_t>(readBack.data.size()) != entry.size_bytes) {
        context.blockers->append(QStringLiteral("APFS %1 could not preserve file %2")
                                      .arg(context.purpose, entry.path));
        return false;
    }
...
bool appendRootDirectoryChildFileInputs(const QString& imagePath,
    ...
    const ApfsBoundedRegularFileAppendContext context{.imagePath = &imagePath,
                                                      ...
                                                      .purpose = QLatin1StringView("directory-file mutation"),
                                                      .inputs = inputs,
                                                      .blockers = blockers};
...
bool appendExistingRootEntriesWithDirectoryFiles(const PartitionApfsFileReadResult& rootListing,
                                                const ApfsDirectoryFilePreservationContext& context) {
    bool foundTargetDirectory = false;
    for (const auto& entry : rootListing.entries) {
        if (entry.directory) {
            const bool isTargetDirectory =
                entry.name.compare(context.targetDirectoryName, Qt::CaseInsensitive) == 0;
        ...
        const ApfsBoundedRegularFileAppendContext fileContext{
    ...
    if (!foundTargetDirectory) {
        context.blockers->append(QStringLiteral(
            "APFS directory-file mutation target root directory was not found"));
        return false;
    }
    return true;
}
...
bool appendExistingRootFileInputs(const QString& imagePath,
    ...
    return appendExistingRootEntries(imagePath,
                                     rootListing,
                                     replacementName,
                                     {inputs, &ignoredDirectories},
                                     blockers);
}
...
bool runRootFileWriteOnScratch(const ApfsRootFileWriteScratch& scratch,
                               QStringList* blockers) {
    uint32_t blockSize = 0;
    uint64_t blockCount = 0;
...
bool runRootFileDeleteOnScratch(const ApfsRootFileDeleteScratch& scratch,
                                QStringList* blockers) {
    uint32_t blockSize = 0;
    uint64_t blockCount = 0;
    if (!readApfsRepairGeometry(scratch.image, &blockSize, &blockCount, blockers)) {
    ...
    const auto rootDirectories = assignedRootDirectoryPayloads(preservedDirectories, preservedInputs.size());
    ...
    if (allocatedBlocksForFiles(preservedFiles, blockSize) > blockCount) {
        blockers->append(QStringLiteral("APFS file-delete preserved data exceeds target container size"));
        return false;
    }
...
bool rootEntryExists(const PartitionApfsFileReadResult& rootListing,
    ...
    for (const auto& entry : rootListing.entries) {
        if (entry.name.compare(name, Qt::CaseInsensitive) == 0) {
            blockers->append(QStringLiteral(
                "APFS generated root-directory create target already exists"));
            return true;
        }
    }
    return false;
...
std::optional<PartitionApfsFileEntry> boundedRootDirectoryEntry(
    ...
    for (const auto& entry : rootListing.entries) {
        ...
        if (!entry.directory) {
            blockers->append(QStringLiteral(
                "APFS %1 certification can mutate only root directories").arg(purpose));
            return std::nullopt;
        }
        return entry;
    }
...
bool appendEmptyRootDirectoryDeleteBlockers(const QString& imagePath,
    ...
    const auto listing =
        PartitionApfsFileSystemReader::listDirectoryFromImage(imagePath, entry.path, 1);
    if (!listing.ok) {
        blockers->append(QStringLiteral("APFS directory-delete could not verify root directory %1: %2")
                             .arg(entry.name, listing.blockers.join(QStringLiteral("; "))));
        return false;
    }
    if (!listing.entries.isEmpty()) {
...
bool runRootDirectoryRewriteOnScratch(const ApfsRootDirectoryRewriteScratch& scratch,
    ...
    if (!appendGeneratedApfsLayoutBlockers(scratch.image,
                                           {.blockSize = blockSize, .blockCount = blockCount},
                                           scratch.createDirectory ? QLatin1StringView("directory-create")
    ...
    if (allocatedBlocksForFiles(files, blockSize) > blockCount) {
        blockers->append(QStringLiteral("APFS directory mutation preserved data exceeds target container size"));
        return false;
    }
    const auto blocks = seedRewriteBlocks({.blockSize = blockSize,
                                           .blockCount = blockCount,
...
bool runVolumeLabelChangeOnTarget(QIODevice* image,
    ...
    if (expectedContainerBytes > 0 &&
        (expectedContainerBytes % blockSize != 0 ||
    ...
    if (!readGeneratedLayoutBlock(image,
                                  geometry,
                                  kApfsFormatVolumeSuperblockBlock,
                                  &volumeBlock,
                                  blockers)) {
    ...
    if (oldVolumeName) {
        *oldVolumeName = nulTerminatedUtf8Field(volumeBlock,
                                                kApfsVolumeNameOffset,
                                                kApfsVolumeNameBytes);
    }
    writeAscii(&volumeBlock,
               kApfsVolumeNameOffset,
               cleanVolumeName.toUtf8(),
               kApfsVolumeNameBytes);
    if (!stampApfsObjectBlock(&volumeBlock, blockers)) {
        return false;
    }
    return writeApfsRepairBlock(image,
                                geometry,
                                kApfsFormatVolumeSuperblockBlock,
                                volumeBlock,
                                blockers);
...
bool appendDirectoryFileWritePayloadBlockers(
    ...
    if (static_cast<uint64_t>(request.file_data.size()) > kApfsMaximumSeedFileBytes) {
        blockers->append(QStringLiteral(
            "APFS directory-file-write data exceeds current certification cap"));
    }
    if (request.options.max_payload_bytes > 0 &&
        static_cast<uint64_t>(request.file_data.size()) > request.options.max_payload_bytes) {
        blockers->append(QStringLiteral(
            "APFS directory-file-write data exceeds configured payload bound"));
    }
    return blockers->isEmpty();
}
...
bool appendDirectoryFilePatchPayloadBlockers(
    ...
    if (static_cast<uint64_t>(request.patch_data.size()) > kApfsMaximumSeedFileBytes) {
        blockers->append(QStringLiteral(
            "APFS directory-file-patch data exceeds current certification cap"));
    }
    if (request.options.max_payload_bytes > 0 &&
        static_cast<uint64_t>(request.patch_data.size()) > request.options.max_payload_bytes) {
        blockers->append(QStringLiteral(
            "APFS directory-file-patch data exceeds configured payload bound"));
    }
    return blockers->isEmpty();
}
...
bool directoryFileWriteReplacesExisting(const PartitionApfsFileReadResult& directoryListing,
    ...
    for (const auto& entry : directoryListing.entries) {
        ...
        if (!entry.regular_file || entry.size_bytes > kApfsMaximumSeedFileBytes) {
            blockers->append(QStringLiteral(
                "APFS directory-file-write can replace only bounded regular files"));
        }
        return true;
    }
    return false;
...
std::optional<PartitionApfsFileEntry> boundedDirectoryFileEntry(
    ...
    for (const auto& entry : directoryListing.entries) {
        ...
        if (!entry.regular_file || entry.size_bytes > kApfsMaximumSeedFileBytes) {
            blockers->append(QStringLiteral(
                "APFS %1 can mutate only bounded directory regular files").arg(purpose));
            return std::nullopt;
        }
        return entry;
    }
...
bool collectRootDirectoryFileInputs(const ApfsRootDirectoryFileScratch& scratch,
    ...
    if (scratch.deleteFile &&
        !appendExistingRootEntriesWithDirectoryFiles(
            scratch.rootListing,
            {.imagePath = &scratch.sourceImagePath,
             .targetDirectoryName = scratch.directoryName,
             .replacementChildName = QString(),
             .preservation = {&payloads->oldInputs, &payloads->oldDirectories},
             .blockers = blockers})) {
        return false;
    }
    if (!appendExistingRootEntriesWithDirectoryFiles(
            scratch.rootListing,
            {.imagePath = &scratch.sourceImagePath,
             .targetDirectoryName = scratch.directoryName,
             .replacementChildName = scratch.fileName,
             .preservation = {&payloads->inputs, &payloads->directories},
             .blockers = blockers})) {
        return false;
    }
    if (!scratch.deleteFile) {
...
bool assignRootDirectoryFilePayloads(const ApfsRootDirectoryFileInputs& inputs,
                                     ...
                                     QStringList* blockers) {
    payloads->rootDirectories =
        assignedRootDirectoryPayloads(inputs.directories, inputs.inputs.size());
    payloads->files = assignedRootFilePayloads(
        inputs.inputs,
        blockSize,
        payloads->rootDirectories,
        blockers);
    const auto oldRootDirectories =
        assignedRootDirectoryPayloads(inputs.oldDirectories, inputs.oldInputs.size());
    payloads->oldFiles = assignedRootFilePayloads(
        inputs.oldInputs,
        blockSize,
        oldRootDirectories,
        blockers);
...
std::optional<PartitionApfsFileReadResult> prepareRootFileWritePlan(
    ...
    if (replacingExisting && !planOptions.allow_compressed_file_mutation) {
        ...
        if (!appendGeneratedSourceLayoutBlockers(context.sourceImagePath,
                                                 QLatin1StringView("file replacement"),
                                                 &layoutBlockers)) {
            context.blockers->append(QStringLiteral(
                "APFS file replacement without compressed/encrypted override requires S.A.K.-generated layout proof"));
...
std::optional<PartitionApfsFileReadResult> prepareRootFileDeletePlan(
    ...
    auto rootListing = rootListingForWrite(context.sourceImagePath, context.blockers);
    const auto target = boundedRootFileEntry(rootListing,
    ...
    QStringList layoutBlockers;
    if (!appendGeneratedSourceLayoutBlockers(context.sourceImagePath,
                                             QLatin1StringView("file deletion"),
                                             &layoutBlockers)) {
        context.blockers->append(QStringLiteral(
            "APFS file deletion requires S.A.K.-generated layout proof"));
        context.blockers->append(layoutBlockers);
        return std::nullopt;
    }
    planOptions.allow_compressed_file_mutation = true;
...
std::optional<PartitionApfsFileReadResult> prepareRootDirectoryFileWritePlan(
    ...
    const auto directoryListing = directoryListingForWrite(context.sourceImagePath,
                                                           *directory,
                                                           QLatin1StringView("directory-file-write"),
                                                           context.blockers);
...
std::optional<PartitionApfsFileReadResult> prepareRootDirectoryFileDeletePlan(
    ...
    if (!appendGeneratedSourceLayoutBlockers(context.sourceImagePath,
                                             ...
                                             &layoutBlockers)) {
        context.blockers->append(QStringLiteral(
            "APFS directory-file deletion requires S.A.K.-generated layout proof"));
        context.blockers->append(layoutBlockers);
        return std::nullopt;
    }
    planOptions.allow_compressed_file_mutation = true;
...
std::optional<PartitionApfsFileReadResult> prepareRootDirectoryFilePatchPlan(
    ...
    if (!appendGeneratedSourceLayoutBlockers(context.sourceImagePath,
                                             ...
                                             &layoutBlockers)) {
        context.blockers->append(QStringLiteral(
            "APFS directory-file patch requires S.A.K.-generated layout proof"));
        context.blockers->append(layoutBlockers);
        return std::nullopt;
    }
    planOptions.allow_compressed_file_mutation = true;
...
std::optional<PartitionApfsFileReadResult> prepareRootDirectoryCreatePlan(
    ...
    QStringList layoutBlockers;
    if (!appendGeneratedSourceLayoutBlockers(context.sourceImagePath,
                                             QLatin1StringView("directory creation"),
                                             &layoutBlockers)) {
        context.blockers->append(QStringLiteral(
            "APFS directory creation requires S.A.K.-generated layout proof"));
        context.blockers->append(layoutBlockers);
        return std::nullopt;
    }
...
std::optional<PartitionApfsFileReadResult> prepareRootDirectoryDeletePlan(
    ...
    const auto target = boundedRootDirectoryEntry(rootListing,
                                                  ...
                                                  context.blockers);
    if (!target.has_value() ||
        !appendEmptyRootDirectoryDeleteBlockers(context.sourceImagePath, *target, context.blockers)) {
        return std::nullopt;
    }

    QStringList layoutBlockers;
    if (!appendGeneratedSourceLayoutBlockers(context.sourceImagePath,
                                             QLatin1StringView("directory deletion"),
                                             &layoutBlockers)) {
        context.blockers->append(QStringLiteral(
            "APFS directory deletion requires S.A.K.-generated layout proof"));
        context.blockers->append(layoutBlockers);
        return std::nullopt;
    }
...
std::optional<PartitionApfsFileReadResult> prepareRootFilePatchPlan(
    ...
    if (!appendGeneratedSourceLayoutBlockers(context.sourceImagePath,
                                             ...
                                             &layoutBlockers)) {
        context.blockers->append(QStringLiteral(
            "APFS file patch requires S.A.K.-generated layout proof"));
        context.blockers->append(layoutBlockers);
        return std::nullopt;
    }
    planOptions.allow_compressed_file_mutation = true;
...
void finalizeRepairResult(PartitionApfsImageRepairResult* result) {
    if (result->blockers.isEmpty()) {
        result->repaired_image_sha256 =
            fileSha256Hex(result->repaired_image_path, &result->blockers);
    }
    result->ok = result->blockers.isEmpty();
    if (!result->ok) {
...
void finalizeFileWriteResult(PartitionApfsImageFileWriteResult* result) {
    if (result->blockers.isEmpty()) {
        result->written_image_sha256 =
            fileSha256Hex(result->written_image_path, &result->blockers);
    }
    result->ok = result->blockers.isEmpty();
    if (!result->ok) {
...
std::optional<PartitionApfsFileEntry> rootDirectoryReadbackEntry(
...
bool appendVolumeLabelReadback(const QString& targetPath,
    ...
    if (rootListing.volume_name != expectedVolumeName) {
        blockers->append(QStringLiteral(
                             "APFS %1 volume-label read-back mismatch: expected '%2', got '%3'")
...
void appendDirectoryCreateReadback(const QString& imagePath,
    ...
    const auto entry = rootDirectoryReadbackEntry(imagePath,
                                                  cleanDirectoryName,
                                                  QLatin1StringView("directory-create"),
                                                  &result->blockers);
    if (!entry.has_value() || !entry->directory) {
        result->blockers.append(QStringLiteral(
            "APFS directory-create read-back did not find the target directory"));
        return;
    }
    const auto childListing = PartitionApfsFileSystemReader::listDirectoryFromImage(
        imagePath,
        QStringLiteral("/%1").arg(cleanDirectoryName),
        1);
    if (!childListing.ok) {
    ...
    if (!childListing.entries.isEmpty()) {
        result->blockers.append(QStringLiteral(
            "APFS directory-create read-back target was not empty"));
    }
}
...
void appendDirectoryDeleteReadback(const QString& imagePath,
    ...
    if (entry.has_value()) {
        result->blockers.append(QStringLiteral(
            "APFS directory-delete read-back still found target directory"));
    }
}
...
void appendDirectoryFileWriteReadback(const QString& imagePath,
    ...
    const auto readBack =
        PartitionApfsFileSystemReader::readFileFromImage(imagePath,
                                                         rootDirectoryFilePath(cleanDirectoryName,
                                                                               cleanFileName),
                                                         static_cast<uint64_t>(expected.size()));
    if (!readBack.ok) {
    ...
    if (readBack.data != expected) {
        result->blockers.append(QStringLiteral(
            "APFS directory-file-write read-back payload mismatch"));
        return;
    }
    result->readback_sha256 = bytesSha256Hex(readBack.data);
}
...
void appendDirectoryFilePatchReadback(const QString& imagePath,
    ...
    if (readBack.data != expected) {
        result->blockers.append(QStringLiteral(
            "APFS directory-file-patch read-back payload mismatch"));
        return;
    }
    result->readback_sha256 = bytesSha256Hex(readBack.data);
}
...
void appendFileDeleteReadback(const QString& imagePath,
    ...
    const auto readBack = PartitionApfsFileSystemReader::readFileFromImage(
        ...
        1);
    if (readBack.ok) {
        result->blockers.append(QStringLiteral("APFS file-delete read-back still found target file"));
    }
}
...
void appendDirectoryFileDeleteReadback(const QString& imagePath,
    ...
    if (readBack.ok) {
        result->blockers.append(QStringLiteral(
            "APFS directory-file-delete read-back still found target file"));
    }
}
...
bool applyRawRootFilePatchPayload(const PartitionApfsRawRootFilePatchRequest& request,
    ...
    if (request.patch_offset_bytes > result->file_bytes ||
        result->patch_bytes > result->file_bytes - request.patch_offset_bytes) {
        result->blockers.append(QStringLiteral(
            "APFS raw file-patch range must stay inside the existing file; use replace for resize"));
        return false;
    }
    std::copy(request.patch_data.cbegin(),
              request.patch_data.cend(),
...
bool applyDirectoryFilePatchPayload(
    const PartitionApfsImageRootDirectoryFilePatchRequest& request,
    const PartitionApfsFileEntry& patchedEntry,
    QByteArray* patchedData,
    PartitionApfsImageFilePatchResult* result) {
    ...
    if (request.patch_offset_bytes > result->file_bytes ||
        result->patch_bytes > result->file_bytes - request.patch_offset_bytes) {
        result->blockers.append(QStringLiteral(
            "APFS directory-file-patch range must stay inside the existing file; use replace for resize"));
        return false;
    }
    std::copy(request.patch_data.cbegin(),
              request.patch_data.cend(),
...
bool applyRawDirectoryFilePatchPayload(
    ...
    if (request.patch_offset_bytes > result->file_bytes ||
        result->patch_bytes > result->file_bytes - request.patch_offset_bytes) {
        result->blockers.append(QStringLiteral(
            "APFS raw directory-file-patch range must stay inside the existing file; use replace for resize"));
        return false;
    }
    std::copy(request.patch_data.cbegin(),
              request.patch_data.cend(),
...
bool runRootFilePatchRewriteOnImage(const ApfsRootFilePatchRewrite& rewrite,
    ...
    QFile image;
    if (!openScratchImage(rewrite.writtenImagePath,
                          QLatin1StringView("file-patch"),
                          &image,
                          blockers)) {
        return false;
...
void appendRawDirectoryFileWriteReadback(const QString& targetPath,
    ...
    if (readBack.data != expected) {
        result->blockers.append(QStringLiteral(
            "APFS raw directory-file-write read-back payload mismatch"));
        return;
    }
    result->readback_sha256 = bytesSha256Hex(readBack.data);
}
...
void appendRawDirectoryFilePatchReadback(const QString& targetPath,
    ...
    if (readBack.data != expected) {
        result->blockers.append(QStringLiteral(
            "APFS raw directory-file-patch read-back payload mismatch"));
        return;
    }
    result->readback_sha256 = bytesSha256Hex(readBack.data);
}
...
void appendRawDirectoryFileDeleteReadback(const QString& targetPath,
    ...
    if (readBack.ok) {
        result->blockers.append(QStringLiteral(
            "APFS raw directory-file-delete read-back still found target file"));
    }
}

void finalizeExistingFormatResult(PartitionApfsImageBuildResult* result,
    ...
    if (result->ok && hashWholeTarget) {
        ...
        result->warnings.append(QStringLiteral(
            "Raw APFS format target full-device SHA-256 skipped; certification uses bounded APFS detection and root listing readback"));
    }
}
...
void appendRawTargetMutationBlockers(const RawTargetMutationBlockersContext& context,
    ...
    if (context.targetPath.trimmed().isEmpty()) {
        blockers->append(QStringLiteral("APFS raw %1 target path is required").arg(context.purpose));
    }
    if (!isWindowsRawDevicePath(context.targetPath)) {
        blockers->append(QStringLiteral("APFS raw %1 requires a Windows raw-device path").arg(context.purpose));
    }
    if (!context.confirmed) {
        blockers->append(QStringLiteral("APFS raw %1 requires explicit target confirmation").arg(context.purpose));
    }
    if (!context.allowRawTarget) {
        blockers->append(QStringLiteral("APFS raw %1 requires explicit raw-target opt-in").arg(context.purpose));
    }
    if (!context.options || context.options->image_only) {
        blockers->append(QStringLiteral("APFS raw %1 requires non-image-only writer options").arg(context.purpose));
    }
    if (context.targetBytes < kMinimumApfsContainerBytes ||
        (context.targetBytes % kSupportedApfsBlockSizeBytes) != 0) {
        blockers->append(QStringLiteral(
            "APFS raw %1 target size must be at least 64 MiB and 4096-byte aligned").arg(context.purpose));
    }
    if (context.targetBytes > static_cast<uint64_t>(std::numeric_limits<qint64>::max())) {
        blockers->append(QStringLiteral("APFS raw %1 target is too large for this platform").arg(context.purpose));
    }
}
...
PartitionApfsImageBuildResult PartitionApfsWriter::buildImageOnlyFormatImage(
    ...
    QFile image;
    if (!createSizedApfsImage(result.image_path,
    ...
    const auto blocks = emptyFormatBlocks(request,
                                          result.plan.target_container_bytes /
...
PartitionApfsImageBuildResult PartitionApfsWriter::formatExistingContainerTarget(
    ...
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS format target: %1")
                                   .arg(openError));
        return result;
    }
    if (!isWindowsRawDevicePath(result.image_path)) {
        ...
        if (openedSize < 0 ||
            static_cast<uint64_t>(openedSize) != request.target_container_bytes) {
            result.blockers.append(QStringLiteral(
    ...
    QStringList writeBlockers;
    if (zeroFormatStaleSignatureRanges(target.get(),
                                       request.target_container_bytes,
                                       &writeBlockers)) {
        const auto blocks = emptyFormatBlocks(request,
                                              result.plan.target_container_bytes /
...
PartitionApfsImageBuildResult PartitionApfsWriter::buildImageOnlyFormatImageWithSeedFile(
    ...
    const auto files = assignedRootFilePayloads({{.fileName = cleanFileName,
...
PartitionApfsImageRepairResult PartitionApfsWriter::repairImageOnlyObjectChecksums(
    ...
    result.warnings.append(QStringLiteral(
        "Generated APFS image repair remains image-only; raw generated-layout repair is exposed only through sak_apfs_writer_cli.exe with explicit selected-target confirmation"));
    ...
    result.plan = planImageOnlyMutation(*detection,
    ...
    QFile image;
    if (!openScratchImage(result.repaired_image_path,
                          QLatin1StringView("repair"),
                          &image,
                          &result.blockers)) {
        return result;
    ...
    if (result.repaired_checksum_blocks == 0 && result.blockers.isEmpty()) {
        result.blockers.append(QStringLiteral(
            "APFS repair did not find any incorrect supported object checksums"));
    }
...
PartitionApfsImageFileWriteResult PartitionApfsWriter::writeImageOnlyRootFile(
    ...
    result.warnings.append(QStringLiteral(
        "Generated APFS file write remains image-only and is not exposed to user actions; raw file-write support requires fsck_apfs/diskutil, crash replay, and hardware evidence"));
...
PartitionApfsImageFileDeleteResult PartitionApfsWriter::deleteImageOnlyRootFile(
    ...
    result.written_image_path = request.written_image_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Generated APFS file delete remains image-only and is not exposed to user actions; raw file-delete support requires fsck_apfs/diskutil, crash replay, and hardware evidence"));
...
PartitionApfsImageFileWriteResult PartitionApfsWriter::writeImageOnlyRootDirectoryFile(
    ...
    result.written_image_path = request.written_image_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Generated APFS directory-file write is limited to S.A.K.-generated single-volume images with bounded root-directory child files"));
    ...
    result.file_bytes = static_cast<uint64_t>(request.file_data.size());
    result.payload_sha256 = bytesSha256Hex(request.file_data);
    if (!appendDirectoryFileWritePayloadBlockers(request,
                                                cleanDirectoryName,
                                                cleanFileName,
                                                &result.blockers)) {
        return result;
    ...
    image.close();
    result.blockers.append(writeBlockers);
    appendDirectoryFileWriteReadback(result.written_image_path,
                                     cleanDirectoryName,
                                     cleanFileName,
                                     request.file_data,
                                     &result);
...
PartitionApfsImageFileDeleteResult PartitionApfsWriter::deleteImageOnlyRootDirectoryFile(
    ...
    result.written_image_path = request.written_image_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Generated APFS directory-file delete is limited to bounded child files in S.A.K.-generated root directories"));
    ...
    image.close();
    result.blockers.append(deleteBlockers);
    appendDirectoryFileDeleteReadback(result.written_image_path,
                                      cleanDirectoryName,
                                      cleanFileName,
                                      &result);
    finalizeFileDeleteResult(&result);
...
PartitionApfsImageFilePatchResult PartitionApfsWriter::patchImageOnlyRootDirectoryFile(
    ...
    result.warnings.append(QStringLiteral(
        "Generated APFS directory-file patch is limited to bounded child files in S.A.K.-generated root directories"));
    ...
    result.directory_name = cleanDirectoryName;
    result.file_name = cleanFileName;
    if (!appendDirectoryFilePatchPayloadBlockers(request,
                                                cleanDirectoryName,
                                                cleanFileName,
                                                &result.blockers)) {
        return result;
    ...
    appendDirectoryFilePatchReadback(result.written_image_path,
                                     cleanDirectoryName,
                                     cleanFileName,
                                     patchedData,
                                     &result);
...
PartitionApfsImageFilePatchResult PartitionApfsWriter::patchImageOnlyRootFile(
    ...
    result.patch_sha256 = bytesSha256Hex(request.patch_data);
    result.warnings.append(QStringLiteral(
        "Generated APFS partial file patch remains image-only and is not exposed to user actions; raw file-patch support requires fsck_apfs/diskutil, crash replay, and hardware evidence"));
...
PartitionApfsImageDirectoryMutationResult PartitionApfsWriter::createImageOnlyRootDirectory(
    ...
    result.written_image_path = request.written_image_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Generated APFS root-directory create remains limited to S.A.K.-generated single-volume images with empty root directories"));
...
PartitionApfsImageDirectoryMutationResult PartitionApfsWriter::deleteImageOnlyRootDirectory(
    ...
    result.written_image_path = request.written_image_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Generated APFS root-directory delete is limited to empty root directories in S.A.K.-generated single-volume images"));
...
PartitionApfsRawFileWriteResult PartitionApfsWriter::writeRawRootFile(
    ...
    result.warnings.append(QStringLiteral(
        "Raw APFS file write remains certifier-only and hidden from user actions until operation-specific queue/apply wiring, fsck_apfs/diskutil validation, and crash replay proof are complete"));
    ...
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw file-write target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList writeBlockers;
...
PartitionApfsRawFileDeleteResult PartitionApfsWriter::deleteRawRootFile(
    ...
    result.target_path = request.target_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Raw APFS file delete remains certifier-only and hidden from user actions until operation-specific queue/apply wiring, fsck_apfs/diskutil validation, and crash replay proof are complete"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw file-delete target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList deleteBlockers;
...
PartitionApfsRawFileWriteResult PartitionApfsWriter::writeRawRootDirectoryFile(
    ...
    result.target_path = request.target_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Raw APFS directory-file write remains certifier-only until fsck_apfs/diskutil validation, crash replay, and hardware proof cover this operation"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw directory-file-write target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList writeBlockers;
    ...
    target->close();
    result.blockers.append(writeBlockers);
    appendRawDirectoryFileWriteReadback(result.target_path,
                                        cleanDirectoryName,
                                        cleanFileName,
                                        request.file_data,
                                        &result);
...
PartitionApfsRawFileDeleteResult PartitionApfsWriter::deleteRawRootDirectoryFile(
    ...
    result.target_path = request.target_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Raw APFS directory-file delete remains certifier-only until fsck_apfs/diskutil validation, crash replay, and hardware proof cover this operation"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw directory-file-delete target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList deleteBlockers;
    ...
    target->close();
    result.blockers.append(deleteBlockers);
    appendRawDirectoryFileDeleteReadback(result.target_path,
                                         cleanDirectoryName,
                                         cleanFileName,
                                         &result);
    result.ok = result.blockers.isEmpty();
...
PartitionApfsRawFilePatchResult PartitionApfsWriter::patchRawRootDirectoryFile(
    ...
    result.patch_sha256 = bytesSha256Hex(request.patch_data);
    result.warnings.append(QStringLiteral(
        "Raw APFS directory-file patch remains certifier-only until fsck_apfs/diskutil validation, crash replay, and hardware proof cover this operation"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw directory-file-patch target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList patchBlockers;
    ...
    target->close();
    result.blockers.append(patchBlockers);
    appendRawDirectoryFilePatchReadback(result.target_path,
                                        cleanDirectoryName,
                                        cleanFileName,
                                        patchedData,
                                        &result);
...
PartitionApfsRawFilePatchResult PartitionApfsWriter::patchRawRootFile(
    ...
    result.patch_sha256 = bytesSha256Hex(request.patch_data);
    result.warnings.append(QStringLiteral(
        "Raw APFS file patch remains certifier-only and hidden from user actions until operation-specific queue/apply wiring, fsck_apfs/diskutil validation, and crash replay proof are complete"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw file-patch target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList patchBlockers;
...
PartitionApfsRawDirectoryMutationResult PartitionApfsWriter::createRawRootDirectory(
    ...
    result.target_path = request.target_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Raw APFS directory create remains certifier-only until fsck_apfs/diskutil validation, crash replay, and hardware proof cover this operation"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw directory-create target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList writeBlockers;
...
PartitionApfsRawDirectoryMutationResult PartitionApfsWriter::deleteRawRootDirectory(
    ...
    result.target_path = request.target_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Raw APFS directory delete remains certifier-only until fsck_apfs/diskutil validation, crash replay, and hardware proof cover this operation"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw directory-delete target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList writeBlockers;
...
PartitionApfsRawVolumeLabelResult PartitionApfsWriter::changeRawVolumeLabel(
    ...
    result.new_volume_name = normalizedApfsVolumeName(request.volume_name);
    result.warnings.append(QStringLiteral(
        "Raw APFS volume-label change is limited to S.A.K. generated APFS layouts; arbitrary Apple APFS metadata mutation remains blocked by layout guards"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw volume-label target: %1")
                                   .arg(openError));
        return result;
    }
    QStringList labelBlockers;
...
PartitionApfsRawRepairResult PartitionApfsWriter::repairRawObjectChecksums(
    ...
    result.target_path = request.target_path.trimmed();
    result.warnings.append(QStringLiteral(
        "Raw APFS checksum repair is limited to S.A.K. generated APFS layouts; arbitrary Apple APFS repair remains blocked by layout guards"));
    ...
    auto target = openFileOrRawDeviceReadWrite(result.target_path, &openError);
    if (!target) {
        result.blockers.append(QStringLiteral("Unable to open APFS raw repair target: %1")
                                   .arg(openError));
        return result;
    }
    ...
    if (readApfsRepairGeometry(target.get(), &blockSize, &blockCount, &repairBlockers)) {
        ...
        } else if (!appendGeneratedApfsLayoutBlockers(
                       target.get(),
                       {.blockSize = blockSize, .blockCount = blockCount},
                       QLatin1StringView("repair"),
                       true,
...
PartitionApfsWritePreflight PartitionApfsWriter::validateImageOnlyExecutionEvidence(
    ...
    if (!evidence.hardware_raw_media_verified) {
        result.warnings.append(QStringLiteral(
            "APFS image-only evidence is not raw-media certification; raw helper execution still requires separate hardware evidence"));
    }
    result.allowed = result.blockers.isEmpty();
    return result;
}
