...
// j_snap_name key: 8-byte (type|OBJ_ID_MASK) header + le16 name_len, then the NUL-terminated name.
constexpr qsizetype kApfsSnapNameKeyNameOffset = 10;
...
// Each snapshot after the first on a quiescent volume adds only its own frozen superblock to
// apfsck's summed v_block_count (+1): the shared omap/catalog nodes were counted by the first.
constexpr uint64_t kApfsSnapshotSubsequentAllocDelta = 1;
...
bool applySpacemanCommitMutations(const ApfsCheckpointCommitContext& ctx,
    ...
    if (ctx.newCibAddr != 0) {
        ...
        const uint64_t ipBmSize = le32(*object, kApfsSpacemanIpBmSizeOffset);
        const qsizetype arrayBase = static_cast<qsizetype>(spacemanCibArrayOffset(ipBmSize));
...
PartitionApfsImageCheckpointCommitResult PartitionApfsWriter::commitImageOnlySnapshotDelete(
    ...
    ApfsInPlaceCheckpointResult commit;
    QStringList commitBlockers;
    if (commitInPlaceSnapshotDelete(&image, request.snapshot_name, &commit, &commitBlockers)) {
        result.previous_xid = commit.previous_xid;
        result.new_xid = commit.new_xid;
        result.checkpoint_map_block = commit.checkpoint_map_block;
        result.superblock_block = commit.superblock_block;
...
PartitionApfsImageCheckpointCommitResult PartitionApfsWriter::commitRawSnapshotDelete(
    ...
    ApfsInPlaceCheckpointResult commit;
    QStringList commitBlockers;
    if (commitInPlaceSnapshotDelete(
...
quint64 PartitionApfsWriter::readGeneratedChunkBitmapAddr(const QString& image_path,
    ...
    // S4b: resolve the chunk-info block that OWNS this chunk (cib chunk_index/126) from the
    // live cib-array, then index the entry local to it (chunk_index % 126). A chunk cib k>0
    // owns is read from cib k's current address, not the live cib 0.
...
quint64 PartitionApfsWriter::readGeneratedSpacemanCibArrayEntry(const QString& image_path,
    ...
    const QVector<uint64_t> addrs =
        readLiveSpacemanCibArray(&image, geometry, nxsb, cib_index + 1, &blockers);
    return addrs.value(static_cast<qsizetype>(cib_index), 0);
}
