...
AdvancedSearchController::AdvancedSearchController(QObject* parent)
    : QObject(parent)
    , m_pattern_library(std::make_unique<RegexPatternLibrary>(nullptr))
{
    loadPreferences();
...
void AdvancedSearchController::startSearch(const SearchConfig& config)
{
    ...
    if (m_preferences.max_search_file_size_mb > 0) {
        effectiveConfig.max_file_size =
            static_cast<qint64>(m_preferences.max_search_file_size_mb) * 1024 * 1024;
    }

    m_worker = std::make_unique<AdvancedSearchWorker>(effectiveConfig);

    // Connect worker signals
    connect(m_worker.get(), &WorkerBase::started,
            this, &AdvancedSearchController::onWorkerStarted);
    connect(m_worker.get(), &WorkerBase::finished,
            this, &AdvancedSearchController::onWorkerFinished);
    connect(m_worker.get(), &WorkerBase::failed,
            this, &AdvancedSearchController::onWorkerFailed);
    connect(m_worker.get(), &WorkerBase::cancelled,
            this, &AdvancedSearchController::onWorkerCancelled);
    connect(m_worker.get(), &WorkerBase::progress,
            this, &AdvancedSearchController::onWorkerProgress);
    connect(m_worker.get(), &AdvancedSearchWorker::resultsReady,
            this, &AdvancedSearchController::onResultsReady);
    connect(m_worker.get(), &AdvancedSearchWorker::fileSearched,
            this, &AdvancedSearchController::onFileSearched);
...
void AdvancedSearchController::onWorkerFinished()
{
    ...
    Q_EMIT statusMessage(
        tr("Search complete: %1 matches in %2 files")
            .arg(m_total_matches).arg(m_total_files),
        5000);
    setState(State::Idle);
}

void AdvancedSearchController::onWorkerFailed(int errorCode,
...
void AdvancedSearchController::onWorkerCancelled()
{
    ...
    Q_EMIT statusMessage(
        tr("Search cancelled: %1 matches found before cancellation")
            .arg(m_total_matches),
        5000);
    setState(State::Idle);
}

void AdvancedSearchController::onWorkerProgress(int current, int total,
...
void AdvancedSearchController::loadPreferences()
{
    ...
    m_preferences.max_results =
        std::max(0, cfg.getValue("advsearch/max_results", 0).toInt());
...
void AdvancedSearchController::savePreferences()
{
    ...
    cfg.setValue("advsearch/max_preview_file_size_mb",
                 m_preferences.max_preview_file_size_mb);
    cfg.setValue("advsearch/max_search_file_size_mb",
                 m_preferences.max_search_file_size_mb);
