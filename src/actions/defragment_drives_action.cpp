...
DefragmentDrivesAction::DefragmentDrivesAction(QObject* parent) : QuickAction(parent) {}

bool DefragmentDrivesAction::isDriveSSD(const QString& drive_letter) {
    Q_ASSERT(!drive_letter.isEmpty());
    QString cmd = QString(
                      "Get-PhysicalDisk | Where-Object {$_.DeviceID -eq (Get-Partition "
...
int DefragmentDrivesAction::analyzeFragmentation(const QString& drive_letter) {
    ...
    ProcessResult proc =
        runProcess("defrag", QStringList() << drive_arg << "/A", sak::kTimeoutProcessLongMs);
...
void DefragmentDrivesAction::scan() {
    ...
    result.summary = fixed_drives > 0 ? QString("Fixed drives detected: %1").arg(fixed_drives)
...
QString DefragmentDrivesAction::executeEnumerateVolumes() {
    ...
    return "# Enterprise Drive Optimization using Optimize-Volume\n"
...
void DefragmentDrivesAction::executeBuildReport(const QString& accumulated_output,
    ...
    if (summary.total_optimized > 0) {
        ...
        result.message =
            QString("All %1 drive(s) skipped (no optimization needed)").arg(summary.total_skipped);
    } else {
...
DefragmentDrivesAction::OptimizationSummary DefragmentDrivesAction::parseOptimizationOutput(
    ...
    while (typeIt.hasNext()) {
        ...
        summary.drive_types.append(
            QString("%1: = %2").arg(match.captured(1), match.captured(2).trimmed()));
    }
    ...
    summary.total_optimized = optMatch.hasMatch() ? optMatch.captured(1).toInt()
                                                  : summary.optimized;
