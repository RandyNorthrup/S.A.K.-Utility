...
static constexpr const char* kDownloadTestUrls[] = {"https://speedtest.tele2.net/10MB.zip",
                                                    "https://proof.ovh.net/files/10Mb.dat",
                                                    "https://speed.hetzner.de/10MB.bin"};
static constexpr const char* kUploadTestUrl = "https://httpbin.org/post";
static constexpr int kConnectivityTimeoutMs = 10'000;
static constexpr int kDownloadTimeoutMs = 120'000;
static constexpr int kUploadTimeoutMs = 45'000;
static constexpr int kLatencyTimeoutMs = 30'000;
static constexpr int kLatencyPingCount = 10;

TestNetworkSpeedAction::TestNetworkSpeedAction(QObject* parent) : QuickAction(parent) {}
...
void TestNetworkSpeedAction::testDownloadSpeed() {
    ...
    QStringList test_urls = {kDownloadTestUrls[0], kDownloadTestUrls[1], kDownloadTestUrls[2]};
    ...
    QString ps_cmd = QString(
                         ...
                         "  try { "
                         "    $start = Get-Date; "
                         "    $response = Invoke-WebRequest -Uri $url -UseBasicParsing -TimeoutSec "
                         "30 -ErrorAction "
                         "Stop; "
                         "    $end = Get-Date; "
...
void TestNetworkSpeedAction::scan() {
    ...
    result.summary = m_has_internet ? "Internet connectivity detected" : "No internet connectivity";
...
void TestNetworkSpeedAction::execute() {
    ...
    if (!m_has_internet) {
        emitFailedResult(QStringLiteral("No internet connection detected"),
