...
void NetworkTransferPanel::setupUi_sourceSection(QVBoxLayout* sourceLayout) {
    ...
    m_customizeUserButton->setToolTip(
        QStringLiteral("Customize data selection for the selected "
                       "user"));
    ...
    m_userTable->setToolTip(
        QStringLiteral("Detected Windows user profiles available for transfer"));
...
void NetworkTransferPanel::setupUi_peerDiscovery(QVBoxLayout* sourceLayout) {
    ...
    m_discoverPeersButton->setToolTip(
        QStringLiteral("Scan the network for available destination "
                       "machines"));
    ...
    m_peerTable->setHorizontalHeaderLabels(
        {tr("Host"), tr("IP"), tr("Mode"), tr("Capabilities"), tr("Last Seen")});
    ...
    m_manualPortSpin->setRange(1024, 65'535);
...
void NetworkTransferPanel::setupUi_securityWidgets() {
    ...
    m_permissionModeCombo->setToolTip(
...
void NetworkTransferPanel::setupUi_destinationSection(QVBoxLayout* destLayout) {
    ...
    m_destinationBaseEdit->setToolTip(
        QStringLiteral("Root directory for receiving transferred "
                       "files"));
    ...
    m_destinationPassphraseEdit->setToolTip(
        QStringLiteral("Passphrase to decrypt incoming "
                       "transfers"));
    ...
    m_startDestinationButton->setToolTip(
        QStringLiteral("Start listening for incoming file "
                       "transfers"));
    destInfoLayout->addWidget(m_startDestinationButton);
    setupUi_orchestratorGroup(destInfoLayout);
    ...
    m_applyRestoreCheck->setToolTip(
        QStringLiteral("Restore transferred data into system user "
                       "profiles"));
...
void NetworkTransferPanel::setupUi_orchestratorGroup(QVBoxLayout* destInfoLayout) {
    ...
    m_autoApproveOrchestratorCheck->setToolTip(
        QStringLiteral("Automatically accept transfer "
...
void NetworkTransferPanel::setupUi_destinationIncoming(QVBoxLayout* destLayout) {
    ...
    m_assignmentQueueTable->setHorizontalHeaderLabels(
        {tr("Deployment"), tr("Job"), tr("User"), tr("Size"), tr("Priority"), tr("Bandwidth")});
    ...
    m_assignmentStatusTable->setHorizontalHeaderLabels(
        {tr("Deployment"), tr("Job"), tr("User"), tr("Status"), tr("Last Event")});
    m_assignmentStatusTable->horizontalHeader()->setSectionResizeMode(
...
void NetworkTransferPanel::setupUi_orchestratorServer(QVBoxLayout* orchestratorLayout) {
    ...
    m_orchestratorListenPortSpin->setToolTip(
        QStringLiteral("Port the orchestrator server listens "
                       "on"));
...
void NetworkTransferPanel::setupUi_orchestratorSources(QVBoxLayout* orchestratorLayout) {
    ...
    m_orchestratorUserTable->horizontalHeader()->setSectionResizeMode(
        2, QHeaderView::ResizeToContents);
...
void NetworkTransferPanel::setupUi_orchestratorDestinations(QVBoxLayout* orchestratorLayout) {
    ...
    m_orchestratorDestTable->setHorizontalHeaderLabels({"?",
                                                        tr("Host"),
                                                        tr("IP"),
                                                        tr("Status"),
                                                        tr("Free Disk"),
    ...
    m_orchestratorDestTable->horizontalHeader()->setSectionResizeMode(
        1, QHeaderView::ResizeToContents);
    m_orchestratorDestTable->horizontalHeader()->setSectionResizeMode(
        2, QHeaderView::ResizeToContents);
    m_orchestratorDestTable->horizontalHeader()->setSectionResizeMode(
...
void NetworkTransferPanel::setupUi_deploymentControls(QVBoxLayout* orchestratorLayout) {
    ...
    m_mappingStrategyCombo->setToolTip(
        QStringLiteral("Strategy for distributing users across "
                       "destinations"));
    ...
    m_globalBandwidthSpin->setRange(0, 100'000);
    ...
    m_perJobBandwidthSpin->setRange(0, 100'000);
...
void NetworkTransferPanel::setupUi_deploymentTemplateActions(QVBoxLayout* controlLayout) {
    ...
    m_saveTemplateButton->setToolTip(
        QStringLiteral("Save the current deployment settings as a "
                       "template"));
    ...
    m_startDeploymentButton->setToolTip(
        QStringLiteral("Begin deploying user profiles to "
                       "destinations"));
...
void NetworkTransferPanel::setupUi_deploymentJobs(QVBoxLayout* orchestratorLayout) {
    ...
    m_jobsTable->setHorizontalHeaderLabels({tr("Job ID"),
                                            tr("Deployment"),
                                            tr("Source User"),
                                            tr("Destination"),
                                            tr("Status"),
...
void NetworkTransferPanel::setupUi_deploymentProgress(QVBoxLayout* orchestratorLayout) {
    ...
    m_recoverDeploymentButton->setToolTip(
        QStringLiteral("Attempt to recover a previously "
    ...
    m_historyTable->setHorizontalHeaderLabels({tr("Deployment"),
                                               tr("Started"),
                                               tr("Completed"),
                                               tr("Total"),
                                               tr("Completed"),
...
void NetworkTransferPanel::setupUi_statusLegend(QVBoxLayout* orchestratorLayout) {
    ...
    warnLabel->setStyleSheet(QString("QLabel { background-color: %1; color: %2; padding: 6px "
                                     "10px; border-radius: 10px; }")
                                 .arg(sak::ui::kColorWarningBadge, sak::ui::kColorTextHeading));
...
void NetworkTransferPanel::setupConnections_sourceSignals() {
    ...
    connect(
        m_customizeUserButton, &QPushButton::clicked, this, &NetworkTransferPanel::onCustomizeUser);
    connect(
        m_discoverPeersButton, &QPushButton::clicked, this, &NetworkTransferPanel::onDiscoverPeers);
    connect(
        m_scanAppsButton, &QPushButton::clicked, this, &NetworkTransferPanel::onScanInstalledApps);
    connect(m_scanAppDataButton, &QPushButton::clicked, this, &NetworkTransferPanel::onScanAppData);
    connect(
        m_scanWifiButton, &QPushButton::clicked, this, &NetworkTransferPanel::onScanWifiProfiles);
    connect(m_scanEthernetButton,
            &QPushButton::clicked,
            this,
...
void NetworkTransferPanel::setupConnections_destinationSignals() {
    ...
    connect(m_startDestinationButton,
            &QPushButton::clicked,
            this,
            &NetworkTransferPanel::onStartDestination);
    connect(m_connectOrchestratorButton,
            &QPushButton::clicked,
            this,
    ...
    connect(m_controller,
            &NetworkTransferController::orchestrationAssignmentReceived,
            this,
            &NetworkTransferPanel::onOrchestrationAssignment);
    connect(m_controller,
            &NetworkTransferController::orchestrationAssignmentPaused,
            this,
    ...
    connect(m_controller,
            &NetworkTransferController::orchestrationAssignmentResumed,
            this,
            [this](const QString& job_id) {
                const QString key = job_id.isEmpty() ? m_activeAssignment.job_id : job_id;
                if (!key.isEmpty()) {
    ...
    connect(m_controller,
            &NetworkTransferController::orchestrationAssignmentCanceled,
            this,
            [this](const QString& job_id) {
                const QString key = job_id.isEmpty() ? m_activeAssignment.job_id : job_id;
                if (!key.isEmpty()) {
...
void NetworkTransferPanel::setupConnections_orchestratorSignals() {
    ...
    connect(m_exportHistoryButton,
            &QPushButton::clicked,
            this,
            &NetworkTransferPanel::onExportDeploymentHistory);
    connect(m_exportSummaryCsvButton,
            &QPushButton::clicked,
            this,
            &NetworkTransferPanel::onExportDeploymentSummaryCsv);
    connect(m_exportSummaryPdfButton,
            &QPushButton::clicked,
            this,
            &NetworkTransferPanel::onExportDeploymentSummaryPdf);
    connect(m_recoverDeploymentButton,
            &QPushButton::clicked,
            this,
            &NetworkTransferPanel::onRecoverLastDeployment);

    setupConnections_orchestratorObject();
}

void NetworkTransferPanel::setupConnections_orchestratorObject() {
    Q_ASSERT(m_orchestrator);
    if (m_orchestrator && m_orchestrator->registry()) {
        connect(m_orchestrator->registry(),
                &DestinationRegistry::destinationRegistered,
                this,
                &NetworkTransferPanel::onOrchestratorDestinationRegistered);
        connect(m_orchestrator->registry(),
                &DestinationRegistry::destinationUpdated,
                this,
                &NetworkTransferPanel::onOrchestratorDestinationUpdated);
        connect(m_orchestrator->registry(),
                &DestinationRegistry::destinationRemoved,
                this,
                &NetworkTransferPanel::onOrchestratorDestinationRemoved);
    }
    if (m_orchestrator) {
        connect(m_orchestrator,
                &MigrationOrchestrator::deploymentProgress,
                this,
                &NetworkTransferPanel::onOrchestratorProgress);
        connect(m_orchestrator,
                &MigrationOrchestrator::deploymentCompleted,
                this,
                &NetworkTransferPanel::onOrchestratorCompletion);
        connect(m_orchestrator,
                &MigrationOrchestrator::aggregateProgress,
                this,
                &NetworkTransferPanel::onAggregateProgress);
        connect(m_orchestrator,
...
void NetworkTransferPanel::setupConnections_controllerSignals() {
    ...
    connect(m_controller,
            &NetworkTransferController::manifestReceived,
            this,
    ...
    connect(
        m_controller, &NetworkTransferController::statusMessage, this, [this](const QString& msg) {
            Q_EMIT logOutput(msg);
            Q_EMIT statusMessage(msg, sak::kTimerStatusDefaultMs);
        });
    connect(
        m_controller, &NetworkTransferController::errorMessage, this, [this](const QString& msg) {
...
void NetworkTransferPanel::setupConnections_parallelManager() {
    ...
    connect(m_parallelManager,
            &ParallelTransferManager::jobUpdated,
            this,
            &NetworkTransferPanel::onJobUpdated);
    connect(m_parallelManager,
            &ParallelTransferManager::jobCompleted,
            this,
            &NetworkTransferPanel::onJobCompleted);
    connect(m_parallelManager,
            &ParallelTransferManager::deploymentProgress,
            this,
            &NetworkTransferPanel::onParallelDeploymentProgress);
    connect(m_parallelManager,
            &ParallelTransferManager::deploymentComplete,
            this,
            &NetworkTransferPanel::onParallelDeploymentCompleted);
    connect(m_parallelManager,
            &ParallelTransferManager::jobBandwidthUpdateRequested,
            this,
            [this](const QString& job_id, int max_kbps) {
                if (m_jobSourceControllers.contains(job_id)) {
                    m_jobSourceControllers.value(job_id)->updateBandwidthLimit(max_kbps);
                }
            });
    setupConnections_parallelJobActions();
}
...
void NetworkTransferPanel::setupConnections_parallelJobActions() {
    ...
    connect(m_parallelManager,
            &ParallelTransferManager::jobResumeRequested,
            this,
            [this](const QString& job_id) {
                if (m_jobSourceControllers.contains(job_id)) {
    ...
    connect(m_parallelManager,
            &ParallelTransferManager::jobCancelRequested,
            this,
            [this](const QString& job_id) {
                if (m_jobSourceControllers.contains(job_id)) {
...
void NetworkTransferPanel::loadSettings_initHistoryManager() {
    ...
    if (historyPath.isEmpty()) {
        ...
        if (!dir.exists()) {
            if (!dir.mkpath(".")) {
                sak::logWarning("Failed to create history directory: {}", historyDir.toStdString());
            }
        }
...
void NetworkTransferPanel::loadSettings_initAssignmentQueue() {
    ...
    if (queuePath.isEmpty()) {
        ...
        if (!dir.exists()) {
            if (!dir.mkpath(".")) {
                sak::logWarning("Failed to create queue directory: {}", queueDir.toStdString());
            }
        }
    ...
    if (!lastTemplatePath.isEmpty() && QFileInfo::exists(lastTemplatePath)) {
        ...
        if (!m_loadedMapping.sources.isEmpty()) {
            ...
            m_templateStatusLabel->setText(
                tr("Loaded template: %1").arg(QFileInfo(lastTemplatePath).fileName()));
        }
    }
...
NetworkTransferPanel::SecurityDialogControls NetworkTransferPanel::buildSecurityDialogControls(
    QGridLayout* layout, QDialog* dialog) {
    Q_ASSERT(m_encryptCheck);
    Q_ASSERT(m_compressCheck);
    ...
    encRow->addWidget(
        new sak::InfoButton(tr("Encrypt all data in transit using AES-256-GCM"), dialog));
    layout->addLayout(encRow, 0, 0);
    auto* cmpRow = new QHBoxLayout();
    cmpRow->addWidget(ctl.compressCheck);
    cmpRow->addWidget(
        new sak::InfoButton(tr("Compress data before sending to reduce bandwidth"), dialog));
    ...
    layout->addWidget(sak::InfoButton::createInfoLabel(
                          tr("Chunk (KB):"), tr("Size of each data block in kilobytes"), dialog),
                      1,
                      0);
    ...
    layout->addWidget(
        sak::InfoButton::createInfoLabel(
            tr("Bandwidth (KB/s):"), tr("Maximum transfer speed in KB/s (0 = unlimited)"), dialog),
        1,
        2);
    ...
    layout->addWidget(sak::InfoButton::createInfoLabel(
                          tr("Permissions:"),
                          tr("How to handle file permissions on the destination machine"),
                          dialog),
                      2,
                      0);
    ...
    layout->addWidget(sak::InfoButton::createInfoLabel(
                          tr("Passphrase:"),
                          tr("Shared passphrase both machines must use for encrypted transfers"),
                          dialog),
                      3,
                      0);
...
void NetworkTransferPanel::buildNetworkSettingsToggles(QFormLayout* layout, QDialog* dialog) {
    ...
    layout->addRow(sak::InfoButton::createInfoLabel(
                       tr("Encryption:"),
                       tr("Encrypt file data in transit using AES-256-GCM for security"),
                       dialog),
                   encryptCheck);
...
void NetworkTransferPanel::buildNetworkSettingsPorts(QFormLayout* layout, QDialog* dialog) {
    ...
    discoveryPort->setRange(1024, 65'535);
    ...
    controlPort->setRange(1024, 65'535);
    controlPort->setValue(config.getNetworkTransferControlPort());
    layout->addRow(sak::InfoButton::createInfoLabel(
                       tr("Control Port:"),
                       tr("TCP port for transfer control commands (handshake, status, cancel)"),
                       dialog),
                   controlPort);
    ...
    chunkSize->setRange(16'384, 4'194'304);
    ...
    layout->addRow(sak::InfoButton::createInfoLabel(
                       tr("Max Bandwidth:"),
                       tr("Limit transfer speed to avoid saturating the network (0 = unlimited)"),
                       dialog),
                   maxBw);
