...
void WifiManagerPanel::onGenerateQrClicked()
{
    ...
    if (sources.size() == 1) {
        // â”€â”€ Single network â€” full wizard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        auto renderWithHeader = [&](bool showHeader) -> QImage {
        ...
        // Page 0: format + header selection
        auto* page0    = new QWidget;
        auto* p0top    = new QHBoxLayout;

        const QImage previewImg = renderWithHeader(true).scaled(180, 180, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        auto* previewLabel = new QLabel(page0);
        previewLabel->setPixmap(QPixmap::fromImage(previewImg));
        ...
        p0layout->setContentsMargins(8, 0, 0, 0);
        p0layout->addWidget(new QLabel("Select export format(s):"));
        ...
        p0layout->addWidget(chkPng);
        p0layout->addWidget(chkPdf);
        p0layout->addWidget(chkJpg);
        p0layout->addWidget(chkBmp);
        p0layout->addSpacing(10);
        auto* headerToggle = new LogToggleSwitch("Location header", optWidget);
        headerToggle->setFixedWidth(180);
        headerToggle->setChecked(true);
        p0layout->addWidget(headerToggle);
        p0layout->addStretch();

        p0top->addWidget(previewLabel);
        p0top->addWidget(optWidget, 1);

        auto* p0outer   = new QVBoxLayout(page0);
        auto* p0buttons = new QHBoxLayout;
        auto* btnCancel0 = new QPushButton("Cancel");
        auto* btnNext    = new QPushButton("Next >");
        btnNext->setDefault(true);
        p0buttons->addStretch();
        p0buttons->addWidget(btnCancel0);
        p0buttons->addWidget(btnNext);
        p0outer->addLayout(p0top);
        p0outer->addLayout(p0buttons);
        stack->addWidget(page0);

        // Page 1: output directory
        auto* page1    = new QWidget;
        auto* p1layout = new QVBoxLayout(page1);
        p1layout->addWidget(new QLabel("Choose output directory:"));
        auto* dirRow   = new QHBoxLayout;
        auto* dirEdit  = new QLineEdit;
        dirEdit->setReadOnly(true);
        dirEdit->setPlaceholderText("Click Browse to select a folder...");
        auto* btnBrowse = new QPushButton("Browse...");
        btnBrowse->setFixedWidth(80);
        dirRow->addWidget(dirEdit, 1);
        dirRow->addWidget(btnBrowse);
        p1layout->addLayout(dirRow);
        auto* subLabel = new QLabel("Files will be saved to: (select a folder first)");
        subLabel->setWordWrap(true);
        subLabel->setStyleSheet("color: #666; font-size: 8pt;");
        p1layout->addWidget(subLabel);
        p1layout->addStretch();
        auto* p1buttons   = new QHBoxLayout;
        auto* btnBack     = new QPushButton("< Back");
        auto* btnCancel1  = new QPushButton("Cancel");
        auto* btnGenerate = new QPushButton("Generate");
        btnGenerate->setDefault(true);
        btnGenerate->setEnabled(false);
        p1buttons->addWidget(btnBack);
        p1buttons->addStretch();
        p1buttons->addWidget(btnCancel1);
        p1buttons->addWidget(btnGenerate);
        p1layout->addLayout(p1buttons);
        ...
        QObject::connect(headerToggle, &LogToggleSwitch::toggled, [&](bool on) {
            const QImage updated = renderWithHeader(on).scaled(180, 180, Qt::KeepAspectRatio, Qt::SmoothTransformation);
            previewLabel->setPixmap(QPixmap::fromImage(updated));
        });
        QObject::connect(btnCancel0, &QPushButton::clicked, &dlg, &QDialog::reject);
        ...
        QObject::connect(btnCancel1, &QPushButton::clicked, &dlg, &QDialog::reject);
        QObject::connect(btnBack,    &QPushButton::clicked, [&]() { stack->setCurrentIndex(0); });
        QObject::connect(btnBrowse, &QPushButton::clicked, [&]() {
            const QString start = dirEdit->text().isEmpty()
                ? QStandardPaths::writableLocation(QStandardPaths::DesktopLocation)
                : dirEdit->text();
            const QString chosen = QFileDialog::getExistingDirectory(&dlg, "Select Output Directory", start);
            if (chosen.isEmpty()) return;
            dirEdit->setText(chosen);
            subLabel->setText(QString("Files will be saved to: %1/%2/").arg(chosen, subName));
        ...
        QObject::connect(btnGenerate, &QPushButton::clicked, [&]() {
            ...
            const bool      showHeader = headerToggle->isChecked();
            const QImage    finalImg   = renderWithHeader(showHeader);
            QStringList     saved;
            ...
            if (chkBmp->isChecked()) savePlain("bmp", "BMP");
            if (chkPdf->isChecked()) {
                const QString pdfPath = outDir + "/" + subName + ".pdf";
                QPdfWriter writer(pdfPath);
                writer.setTitle(ssid.isEmpty() ? "WiFi QR Code" : ssid + " WiFi QR Code");
                ...
                const QImage scaled   = finalImg.scaled(side, side, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        ...
        // â”€â”€ Multiple networks â€” batch export dialog â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        QDialog dlg(this);
        dlg.setWindowTitle(QString("Batch QR Export (%1 networks)").arg(sources.size()));
        dlg.setMinimumWidth(400);

        auto* layout = new QVBoxLayout(&dlg);
        layout->addWidget(new QLabel(QString("Generate QR codes for %1 selected networks:").arg(sources.size())));
        layout->addSpacing(6);
        ...
        auto* chkJpg = new QCheckBox("JPG");
        auto* chkBmp = new QCheckBox("BMP");
        layout->addWidget(chkPng);
        layout->addWidget(chkPdf);
        layout->addWidget(chkJpg);
        layout->addWidget(chkBmp);
        layout->addSpacing(8);
        ...
        auto* dirEdit  = new QLineEdit;
        dirEdit->setReadOnly(true);
        dirEdit->setPlaceholderText("Click Browse...");
        auto* btnBrowse = new QPushButton("Browse...");
        btnBrowse->setFixedWidth(80);
        dirRow->addWidget(dirEdit, 1);
        dirRow->addWidget(btnBrowse);
        ...
        QObject::connect(btnBrowse, &QPushButton::clicked, [&]() {
            ...
            if (chosen.isEmpty()) return;
            dirEdit->setText(chosen);
            subLabel->setText(QString("One subfolder will be created per network under: %1").arg(chosen));
        ...
        QObject::connect(btnGen, &QPushButton::clicked, [&]() {
            ...
            for (const WifiConfig& cfg : sources) {
                ...
                // Render with optional location header
                QImage img = generateQrImage(cfgPayload);
                if (showHeader && !cfg.location.isEmpty()) {
                    constexpr int HEADER_H = 52;
                    QImage out(img.width(), img.height() + HEADER_H, QImage::Format_RGB32);
                    out.fill(Qt::white);
                    QPainter p(&out);
                    QFont f = p.font(); f.setPointSize(14); f.setBold(true);
                    p.setFont(f); p.setPen(Qt::black);
                    p.drawText(QRect(0, 0, out.width(), HEADER_H), Qt::AlignHCenter | Qt::AlignVCenter, cfg.location);
                    p.drawImage(0, HEADER_H, img);
                    p.end();
                    img = std::move(out);
                }

                bool anySaved = false;
                if (chkPng->isChecked()) { img.save(outDir + "/" + subName + ".png", "PNG"); anySaved = true; }
                if (chkJpg->isChecked()) { img.save(outDir + "/" + subName + ".jpg", "JPEG"); anySaved = true; }
                if (chkBmp->isChecked()) { img.save(outDir + "/" + subName + ".bmp", "BMP"); anySaved = true; }
                if (chkPdf->isChecked()) {
                    const QString pdfPath = outDir + "/" + subName + ".pdf";
                    QPdfWriter writer(pdfPath);
                    writer.setTitle(cfg.ssid + " WiFi QR Code");
                    writer.setPageSize(QPageSize(QPageSize::A4));
                    writer.setResolution(150);
                    const QRect  pageRect = writer.pageLayout().paintRectPixels(writer.resolution());
                    const int    side     = std::min(pageRect.width(), pageRect.height());
                    const QImage scaled   = img.scaled(side, side, Qt::KeepAspectRatio, Qt::SmoothTransformation);
                    QPainter painter(&writer);
                    painter.drawImage((pageRect.width() - scaled.width()) / 2, 0, scaled);
                    painter.end();
                    anySaved = true;
            ...
            const QString msg = failed > 0
                ? QString("Batch QR: saved %1 network(s) to %2 (%3 failed).").arg(saved).arg(baseDir).arg(failed)
...
void WifiManagerPanel::onConnectWithPhoneClicked()
{
    ...
    if (sources.size() == 1) {
        // â”€â”€ Single network â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const WifiConfig& cfg     = sources.first();
        const QString     payload = buildWifiPayloadFromConfig(cfg);
        ...
        // â”€â”€ Multiple networks â€” navigable QR dialog â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        QDialog dlg(this);
...
void WifiManagerPanel::onScanNetworksClicked()
{
    ...
    if (!proc.waitForFinished(8000)) {
        Q_EMIT statusMessage("Timed out running netsh. Is netsh available?", 4000);
        return;
    }
    ...
    for (const QString& name : profileNames) {
        QProcess p2;
        p2.start("netsh", QStringList{"wlan", "show", "profile", "name=" + name, "key=clear"});
...
void WifiManagerPanel::onAddToWindowsClicked()
{
    ...
    for (int row : checkedRows) {
        ...
        // Write profile to a uniquely-named temp file
