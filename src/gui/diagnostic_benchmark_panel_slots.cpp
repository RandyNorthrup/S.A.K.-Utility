...
void DiagnosticBenchmarkPanel::onHardwareScanComplete(const HardwareInventory& inventory) {
    Q_ASSERT(m_hw_cpu_label);
    Q_ASSERT(m_hw_memory_label);
    setOperationRunning(false);

    m_hw_cpu_label->setText(QString("%1 (%2C/%3T) @ %4 MHz")
                                .arg(inventory.cpu.name)
                                .arg(inventory.cpu.cores)
                                .arg(inventory.cpu.threads)
                                .arg(inventory.cpu.max_clock_mhz));

    m_hw_memory_label->setText(QString("%1 (%2/%3 slots)")
    ...
    if (!inventory.gpus.isEmpty()) {
        ...
        for (const auto& gpu : inventory.gpus) {
            gpu_lines.append(QString("%1 (%2)").arg(gpu.name, formatBytes(gpu.vram_bytes)));
        }
        m_hw_gpu_label->setText(gpu_lines.join("; "));
    }

    m_hw_motherboard_label->setText(QString("%1 %2 (BIOS: %3)")
                                        .arg(inventory.motherboard.manufacturer,
                                             inventory.motherboard.product,
                                             inventory.motherboard.bios_version));

    m_hw_os_label->setText(QString("%1 %2 (Build %3)")
...
void DiagnosticBenchmarkPanel::populateDiskDriveCombo(const HardwareInventory& inventory) {
...
void DiagnosticBenchmarkPanel::addDiskPartitionsToCombo(const StorageDeviceInfo& disk) {
    for (const auto& part : disk.partitions) {
        ...
        m_disk_drive_combo->addItem(
            QString("%1 (%2 - %3)")
                .arg(part.drive_letter, disk.model, formatBytes(disk.size_bytes)),
            part.drive_letter + "\\");
    }
}
...
void DiagnosticBenchmarkPanel::onSmartAnalysisComplete(const QVector<SmartReport>& reports) {
    ...
    logMessage(QString("SMART analysis complete: %1 drive(s) analyzed").arg(reports.size()));
...
void DiagnosticBenchmarkPanel::onCpuBenchmarkComplete(const CpuBenchmarkResult& result) {
...
void DiagnosticBenchmarkPanel::onDiskBenchmarkComplete(const DiskBenchmarkResult& result) {
...
void DiagnosticBenchmarkPanel::onMemoryBenchmarkComplete(const MemoryBenchmarkResult& result) {
    ...
    m_mem_latency_label->setText(QString("Random Latency: %1 ns | Alloc Stress: %2 ops/s | "
                                         "Max Contiguous: %3 MB")
...
void DiagnosticBenchmarkPanel::onStressTestComplete(const StressTestResult& result) {
...
void DiagnosticBenchmarkPanel::onGenerateReportClicked() {
    ...
    Q_ASSERT(m_controller);
    const auto dir = QFileDialog::getExistingDirectory(this, "Select Report Output Directory");
    if (dir.isEmpty()) {
        return;
    }
