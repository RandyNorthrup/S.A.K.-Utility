...
rem disk. It used to hardcode "-TargetDiskNumber 2 -Force", so double-clicking it destroyed
rem whatever was disk 2 on that machine with no prompt and no way to say otherwise. The disk
...
"%PS%" -NoProfile -ExecutionPolicy Bypass -Command "Start-Process -FilePath '%PS%' -Verb RunAs -ArgumentList '-NoProfile -NoExit -ExecutionPolicy Bypass -File \"\\vboxsvr\sakrepo\scripts\run_partition_manager_bitlocker_mutation_external_gate.ps1\" -TargetDiskNumber %~1 -Force'"
exit /b %ERRORLEVEL%
