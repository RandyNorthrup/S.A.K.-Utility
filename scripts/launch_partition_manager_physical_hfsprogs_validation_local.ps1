...
.DESCRIPTION
    ...
    selected expendable partition and writes JSON evidence. The runner is pinned
    by hash, the resolved disk identity is shown before the UAC prompt, the
    elevated console is captured to a log, and success requires fresh evidence
    on disk.
#>
...
param(
    ...
    [ValidateRange(-1, 4095)] [int]$PartitionNumber = -1,
...
if ($FileSystems.Count -eq 0) {
    throw "FileSystems is empty. Pass at least one of HFS+, HFSX; an empty set would certify nothing."
}
...
if ($PartitionNumber -lt 0) {
    Write-Host "PartitionNumber was not pinned; the elevated runner selects the expendable partition itself."
}
else {
    $partition = Get-Partition -DiskNumber $DiskNumber -PartitionNumber $PartitionNumber -ErrorAction Stop
    Write-Host "Target partition $PartitionNumber : type $($partition.Type) offset $($partition.Offset) size $($partition.Size) bytes letter '$($partition.DriveLetter)'."
}
...
$logRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("sak-hfsprogs-launch-" + [Guid]::NewGuid().ToString("N"))
...
$runnerArgs = "-DiskNumber $DiskNumber -PartitionNumber $PartitionNumber -ProjectRoot $(Quote-PowerShellLiteral $ProjectRoot) -EvidenceRoot $(Quote-PowerShellLiteral $EvidenceRoot) -FileSystems $fileSystemsLiteral -Force"
...
if ($process.ExitCode -ne 0) {
    throw "Elevated physical HFS tool validation exited with code $($process.ExitCode).$(Get-ElevatedLogTail -Path $childLog)"
}
