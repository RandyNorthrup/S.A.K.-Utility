...
.DESCRIPTION
    ...
    elevated child a hash-pinned command. TargetDiskNumber is forwarded only
    when supplied so the operator can pin the target disk instead of leaving
    the runner to auto-select.
#>
...
$sourceScript = Join-Path $sharedRoot "scripts\run_partition_manager_offline_uefi_bcd_break.ps1"
...
$localScript = Join-Path $stageDirectory.FullName "run_partition_manager_offline_uefi_bcd_break.ps1"
...
$runnerArgs = "-OutputRoot $(Quote-PowerShellLiteral $outputRoot) -Force"
...
if ($process.ExitCode -ne 0) {
    throw "Elevated offline UEFI break exited with code $($process.ExitCode).$(Get-ElevatedLogTail -Path $childLog)"
}
Assert-FreshEvidence -Root $outputRoot -SinceUtc $launchUtc

Write-Host "Elevated offline UEFI break completed. Console log: $childLog"
