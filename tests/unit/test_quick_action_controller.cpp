...
public:
    ...
    QString description() const override {
        return m_name + " description";
    }
    QIcon icon() const override { return {}; }
    ActionCategory category() const override {
        return m_category;
    }
...
void TestQuickActionController::testRegisterAndGet() {
    auto action = std::make_unique<ControllerStubAction>(
        "TestAction1");
    const QString name =
        m_ctrl->registerAction(std::move(action));

    QCOMPARE(name, "TestAction1");
    QVERIFY(m_ctrl->getAction("TestAction1") != nullptr);
    QCOMPARE(
        m_ctrl->getAction("TestAction1")->name(),
        "TestAction1");
}
...
void TestQuickActionController::testGetAllActions() {
    m_ctrl->registerAction(
        std::make_unique<ControllerStubAction>("A1"));
    m_ctrl->registerAction(
        std::make_unique<ControllerStubAction>("A2"));
    m_ctrl->registerAction(
        std::make_unique<ControllerStubAction>("A3"));
...
void TestQuickActionController::testNoDuplicateRegistration() {
    m_ctrl->registerAction(
        std::make_unique<ControllerStubAction>("Dup"));
    ...
    auto action2 =
        std::make_unique<ControllerStubAction>("Dup");
    const QString name2 =
        m_ctrl->registerAction(std::move(action2));
...
void TestQuickActionController::testGetByCategory() {
    m_ctrl->registerAction(
        std::make_unique<ControllerStubAction>(
            "Maint1",
    ...
    auto maint = m_ctrl->getActionsByCategory(
        QuickAction::ActionCategory::Maintenance);
    QCOMPARE(static_cast<int>(maint.size()), 2);

    auto backup = m_ctrl->getActionsByCategory(
        QuickAction::ActionCategory::QuickBackup);
...
void TestQuickActionController::testGetByCategoryEmpty() {
    ...
    auto troubleshoot = m_ctrl->getActionsByCategory(
