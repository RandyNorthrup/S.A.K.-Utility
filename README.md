...
- Native Qt functional clone of the Files app (files-community/Files) shell: grouped sidebar navigation with compact rail, command toolbar in the Files order with flyouts, back/forward/up navigation with history flyouts, three-mode omnibar (breadcrumb path editing, command palette, inline search), details/list/grid/cards/columns/adaptive view modes, exact context menus, the full Files hotkey map, status-center transfer cards, bottom status strip, and a persistent details/preview info pane
...
- The functional Files clone is specified and certified in [docs/archive/FILE_EXPLORER_FILES_CLONE_PLAN.md](docs/archive/FILE_EXPLORER_FILES_CLONE_PLAN.md) (C8 record: raw APFS/HFS+ matrix proofs, a physical 8TB destructive matrix Apple-kernel-certified via fsck_apfs, and a screenshot A/B pass against Files 4.1.1); the earlier milestone history lives in [docs/archive/FILE_MANAGEMENT_EXPLORER_FILES_LIKE_PLAN.md](docs/archive/FILE_MANAGEMENT_EXPLORER_FILES_LIKE_PLAN.md). Cloud drives, FTP, Git integration, and third-party integrations are explicitly out of scope for the current milestone.
