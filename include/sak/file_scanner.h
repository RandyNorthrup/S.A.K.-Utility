...
using scan_progress_callback =
    std::function<void(std::size_t files_processed, std::uintmax_t total_size_processed)>;
...
public:
    ...
    [[nodiscard]] auto scan(const std::filesystem::path& root_path,
                            const scan_options& options,
                            std::stop_token stop_token = {})
        -> std::expected<scan_statistics, error_code>;
    ...
    [[nodiscard]] auto scanAndCollect(const std::filesystem::path& root_path,
                                      const scan_options& options,
                                      std::stop_token stop_token = {})
    ...
    [[nodiscard]] static auto listFiles(const std::filesystem::path& root_path,
                                        bool recursive = true)
        -> std::expected<std::vector<std::filesystem::path>, error_code>;
    ...
    [[nodiscard]] static auto findFiles(const std::filesystem::path& root_path,
...
private:
    ...
    auto scanDirectoryRecursive(const std::filesystem::path& current_path,
                                const scan_options& options,
                                scan_statistics& stats,
                                std::size_t current_depth,
    ...
    auto processScannedEntry(const std::filesystem::directory_entry& entry,
                             const scan_options& options,
                             scan_statistics& stats,
                             std::size_t current_depth,
                             std::stop_token stop_token) -> std::expected<void, error_code>;
