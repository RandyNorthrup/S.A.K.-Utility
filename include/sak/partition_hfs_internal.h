...
/// @brief Shared HFS+/HFSX engine (HfsReader) used by both the read-only
///        browser (PartitionHfsFileSystemReader) and the writer
///        (PartitionHfsFileSystemWriter). Internal to Partition Manager;
///        not a public API. The engine lives in a header-only translation
///        unit so the thin public wrappers can live in separate .cpp files.
///        Follow-up (tracked in docs/APFS_HFS_FULL_DRIVER_WRITE_PLAN.md):
///        promote hot subsystems to a compiled-once sak::hfs_detail module.
