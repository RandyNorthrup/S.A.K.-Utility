...
[[nodiscard]] inline QString formatBytes(qint64 bytes) {
    if (bytes <= 0) return QStringLiteral("0 bytes");
    ...
    if (bytes >= kTB) return QString("%1 TB").arg(static_cast<double>(bytes) / kTB, 0, 'f', 2);
    if (bytes >= kGB) return QString("%1 GB").arg(static_cast<double>(bytes) / kGB, 0, 'f', 2);
    if (bytes >= kMB) return QString("%1 MB").arg(static_cast<double>(bytes) / kMB, 0, 'f', 1);
    if (bytes >= kKB) return QString("%1 KB").arg(static_cast<double>(bytes) / kKB, 0, 'f', 1);
