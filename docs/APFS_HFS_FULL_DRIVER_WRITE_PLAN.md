...
| H2 🟡 | Streaming B-tree engine: arbitrary depth/width (H-b) | Real-world large directories | `fsck_hfs` clean on >256-leaf + depth-4 synthetic and on an Apple-written large catalog — **SYNTHETIC GATE MET (2026-06-16)**: the catalog B-tree load (recursive by height) + bottom-up index rebuild were already depth-general; raised the depth/leaf caps (3->8 / 256->16384) and made withCatalogNodePoolGrowth retry with a doubling free-node target (the rebuild needs ~whole-index free nodes, not a single split path - was failing at 734 files). Bundled `fsck_hfs -n -f` "appears to be OK" on a `newfs_hfs`(4096-node) volume grown to tree_depth 5 / >256 leaf nodes via 1500 long-named inserts; 124/124 unit tests. **macOS VM kernel evidence (2026-06-16):** the macOS Sequoia kernel AUTO-MOUNTED the depth-5 SAKH2 volume (Apple fsck_hfs: "mounted with write access") and `ls /Volumes/SAKH2` READ all 1500 files (f-0000..f-1499) from the deep catalog - full kernel acceptance of the deep tree. Evidence hfs.h2-deep-catalog-kernel-mount-rw.png + -read-1500-files.png. (In-VM offline Apple fsck blocked by a recovery display-sleep; redundant with the bundled diskdev_cmds-port fsck which already passed. RW touch failed = recovery mounts UNJOURNALED HFS+ read-only, not a defect; a `newfs_hfs -J` journaled volume in a fresh VM would touch-RW.) **REMAINING:** in-VM offline Apple fsck + RW write on a journaled deep volume (formal confirmation); mutate an Apple-WRITTEN large catalog (not just S.A.K. newfs+insert); map-node growth (>~32000 nodes) still fail-closed (far beyond the gate). See [[h2-hfs-btree-scope]] |
