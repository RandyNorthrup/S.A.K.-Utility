...
| H2 🟡 | Streaming B-tree engine: arbitrary depth/width (H-b) | Real-world large directories | `fsck_hfs` clean on >256-leaf + depth-4 synthetic and on an Apple-written large catalog — **SYNTHETIC GATE MET (2026-06-16)**: the catalog B-tree load (recursive by height) + bottom-up index rebuild were already depth-general; raised the depth/leaf caps (3->8 / 256->16384) and made withCatalogNodePoolGrowth retry with a doubling free-node target (the rebuild needs ~whole-index free nodes, not a single split path - was failing at 734 files). Bundled `fsck_hfs -n -f` "appears to be OK" on a `newfs_hfs`(4096-node) volume grown to tree_depth 5 / >256 leaf nodes via 1500 long-named inserts; 124/124 unit tests. **REMAINING:** macOS VM kernel RW mount + Apple `fsck_hfs` on the deep-tree volume; mutate an Apple-WRITTEN large catalog (not just S.A.K. newfs+insert); map-node growth (>~32000 nodes) still fail-closed (far beyond the gate). See [[h2-hfs-btree-scope]] |
