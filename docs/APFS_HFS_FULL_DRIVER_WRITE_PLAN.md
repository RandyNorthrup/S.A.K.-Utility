...
> streaming split.
>
> **H3 (2026-06-21) 🟡 EXIT GATE MET, kernel-mount flourish pending.** Delete-side
> underfull leaf merge/rebalance implemented + adversarially reviewed (0 defects) +
> unit-proven (decisive merge collapse + 300-op randomized churn) + Apple `fsck_hfs`
> (bundled diskdev_cmds 540.1) clean on a churned volume that merged depth 4→3 /
> 577→47 nodes. The H1/H2-style macOS-kernel-RW-mount + Apple `fsck_hfs` 683 step is
> blocked this session by a qemu 11.0 macOS-HID-injection regression (not an H3 code
> issue). Next: fix the VM HID to finish H3's kernel mount, then A3 / H4.**
