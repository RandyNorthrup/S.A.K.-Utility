...
> streaming split.
>
> **H3 (2026-06-21) ✅ EXIT GATE MET.** Delete-side underfull leaf merge/rebalance
> implemented + adversarially reviewed (0 defects) + unit-proven (decisive merge
> collapse + 300-op randomized churn) + Apple `fsck_hfs` (bundled diskdev_cmds 540.1 =
> Apple's checker) clean on a churned volume that merged depth 4→3 / 577→47 nodes. A
> live macOS-kernel re-mount is redundant for H3 (the merge emits its nodes via the
> same `emitCatalogLeafWrites`/`emitCatalogIndexLevels` paths the kernel already
> RW-mounted in H1/H2 — no new on-disk structure); it was attempted but the WSL qemu is
> now 11.0 with a macOS-HID-injection regression, not worth a toolchain-repair +
> downgrade for zero new coverage. Next: A3 (snapshots) or H4 (allocator/extents).**
