...
| A-b | True in-place copy-on-write checkpoint mutation of arbitrary Apple containers | **🟡 IN PROGRESS 2026-06-14** — increment 1 (re-checkpoint engine: ring advance + ephemeral re-emit + checksum) **DONE**: `commitImageOnlyCheckpoint` advances xid in place; Apple `fsck_apfs` OK + kernel RW auto-mount + apfs.kext continued the S.A.K. ring (xid 4→6). Increment 2 (fstree/omap COW + spaceman bitmap = real file insert) pending | Mutate existing container in place: checkpoint ring, ephemeral commit, omap COW, xid advance, reaper, free-queue | COW checkpoint engine over the existing NXSB/checkpoint descriptor area | `fsck_apfs` + kernel RW mount after each commit; crash proof — **increment 1 DONE** (`artifacts/…/apfs-a2-increment1-checkpoint-commit.report.json`) |
