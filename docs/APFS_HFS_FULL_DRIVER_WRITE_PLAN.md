...
| A7 🟡 | Resize + clones + sparse + dir hardlinks + xattr/ACL + sealed-volume policy (A-g, A-h) | Remaining native ops | `fsck_apfs` + kernel mount per op; `diskutil` resize cross-check — **IN PROGRESS 2026-06-27: 3 of 6 capabilities on the certified COW engine, host-apfsck-clean + unit-certified (macOS-kernel mount is the next external pass for each).** (1) **arbitrary xattr/ACL** (`19335e6`): a file insert attaches any named attributes (`com.apple.system.Security` ACL → HAS_SECURITY_EA, `com.apple.FinderInfo` → HAS_FINDER_INFO, user xattrs); reader surfaces them. (2) **sparse files** (`19335e6`): trailing-hole insert → `INODE_IS_SPARSE` + `INO_EXT_TYPE_SPARSE_BYTES` xfield + explicit phys-0 hole extent (apfsck: extents consecutive, `d_bytes==alloced_size`, `d_sparse_bytes==i_sparse_bytes`); reader zero-fills. Host **apfsck CLEAN (exit 0)** on a container with a both-sparse-and-xattr file. Also fixed the documented **A2 DSTREAM-xfield apfsck divergence** (flags 0x08→0x20 = Apple's XF_SYSTEM_FIELD; kernel-safe) + two latent reader xfield-alignment bugs. (3) **sealed-volume policy** (`4316913`): every COW commit fails closed on a signed-system volume (`APFS_INCOMPAT_SEALED_VOLUME` or `apfs_integrity_meta_oid`), typed override documented. **REMAINING: file clones (A-h, extentref refcnt=2 + dstream/inode WAS_EVER_CLONED — needs extentref-tree refcount accounting in the COW commit); directory hardlinks (A-h); container/volume resize (A-g, spaceman reshape — the largest piece). + macOS-kernel mount + `fsck_apfs` external cert for all of A7.** |
