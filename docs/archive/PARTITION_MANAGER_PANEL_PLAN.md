...
### Implementation Status - Current As Of 2026-06-05 UTC / 2026-06-05 PDT
...
- Added storage inventory parsing, layout hashing, unallocated-region discovery, system/boot/EFI/MSR/recovery classification, resilient direct/drive-letter/access-path volume lookup, deterministic EFI/MSR file-system labels when Windows hides mapped volumes, read-only raw signature detection for NTFS, exFAT, FAT12/16/32, ext2/ext3/ext4, XFS, Btrfs, Linux swap, HFS+, HFSX, and APFS when Windows has no file-system label, XFS/Btrfs/APFS/ext/HFS read-only metadata capture, allowlisted elevated raw-probe retry when the normal app process cannot open the physical drive read-only, storage reliability counters, BitLocker/dirty-bit collection, and read-only unsupported-state flags.
...
- Removed menu-bar creation, fixed the left Actions and Wizards pane width, changed Wizards and Partition Operations entries to compact icon text links, moved scrolling inside the Partition Operations group above the visible Pending Operations queue, removed the redundant sidebar layout-preview box, kept the before/after layout diff in the final Apply review, removed row-number gutters and table cell borders while adding disk separators to match the AOMEI partition list, removed the extra disk-map frame, tightened disk-map padding to a 1 px gutter, rounded the disk row/tile container corners, added explicitly painted GPT/Primary, Logical, Simple, Spanned, Striped, Mirrored, RAID5, and dark-gray Unallocated disk-map legend colors, verified those legend colors against rendered swatch and segment pixels, moved type color to the inner usage bar while keeping the outer segment shell neutral, limited partition highlight to selected partitions, added whole-disk row highlight for selected disks, synchronized mouse/keyboard disk-map segment selection with table selection in both directions, attached target-aware right-click menus to the table and disk map, made disk tiles and partition segments the same height, kept Redo disabled until Undo creates a redoable action, used fixed-size Icons8 SVG ribbon buttons with dedicated disabled SVG mode mappings and a dedicated Dry Run icon, made the color-coded rounded disk map resizable upward into table space, moved inventory refresh to an explicit Scan Disks/Refresh Disks control so adding or opening the panel does not start storage inventory, added resilient EFI/MSR file-system labeling so hidden protected partitions no longer show blank file systems when Windows returns no mapped volume, and added detection-source tooltips/properties for Windows, protected-partition, and raw-signature labels.
- Added [Partition Manager Cross-File-System Plan](PARTITION_MANAGER_CROSS_FILESYSTEM_PLAN.md) for portable Linux/HFS+/APFS detection, read, format, and repair work. Milestone A/B detection and read-only browse/extract are implemented: raw ext/XFS/Btrfs/APFS/HFS metadata is visible in Properties and Inspect Non-Windows File System, APFS space-manager parsing reports real free bytes when the spaceman object is visible, XFS/Btrfs/APFS can open an original read-only metadata consistency report from Check Non-Windows File System when captured sanity data exists, HFS+/HFSX can open an original read-only catalog consistency and attributes B-tree key scan report from Check Non-Windows File System, physical-drive probe reads retry through the allowlisted elevated `ReadPartitionProbe` task when the normal process cannot open the disk read-only, ext2/ext3/ext4, HFS+/HFSX, and APFS directory listing is available through Browse Non-Windows File System, ext/HFS+/APFS selected-file extraction is read-only, ext/HFS+/APFS bounded recursive directory export is read-only, HFS+ data/resource-fork extraction and selected attribute-value extraction are available for selected files, HFS+ exports resource forks as `.rsrc` sidecars, HFS+ catalog/data/resource/attribute extents-overflow records are followed for reads, and APFS follows checkpoint/object-map/volume-object-map/inode/file-extent records for normal unencrypted/uncompressed selected files and bounded directory exports. The approved e2fsprogs 1.47.4 bundle includes `e2fsck`, `mke2fs`, and `resize2fs`; `e2fsck -n -f` is the immediate read-only ext check, and confirmed ext create, format, repair, adjacent grow, and same-start shrink now queue through Pending Operations, safety validation, Apply review, elevated PowerShell execution, manifest/hash verification, and operation reporting. The approved hfsprogs 540.1.linux3-6+sak-msys bundle includes `newfs_hfs` and `fsck_hfs`; HFS+/HFSX create, format, and repair now queue through sparse staging in the Non-Windows workflow with raw-target identity, safety validation, Apply review, manifest/hash verification, and explicit confirmation, while read-only HFS checks use original parser reports. The HFS File action now queues selected HFS+/HFSX raw-partition data-fork, resource-fork, bounded initial-extent allocation growth, truncate, empty-file/folder create/delete, bounded file create with data, allocated-file delete with optional released-block zeroing, bounded folder-tree delete with optional released-block zeroing, inline-attribute, fork-backed-attribute replacement, and bounded fork-backed-attribute growth mutations through sparse staging, `sak_hfs_writer_cli.exe`, staged `fsck_hfs` repair/check, changed-range copyback, safety validation, Apply review, operation reporting, and explicit confirmation. Linux swap create/format now queues through Create Partition or Format Partition with page-size selection, original SWAPSPACE2 header generation, Apply review, and explicit confirmation. APFS generated create/format, generated-layout checksum repair, generated-layout root-file write/patch/delete, generated-layout empty root-directory create/delete, generated-layout root-directory child-file write/patch/delete, and generated-layout volume-label change now queue through `sak_apfs_writer_cli.exe`, raw-target identity validation, Apply review, explicit confirmation, and helper JSON reports; generated-image APFS root-file create/replace/byte-range patch/delete, empty root-directory create/delete, root-directory child-file write/patch/delete, and volume-label change are helper-exposed with payload/read-back, deleted-file, directory-empty, old/new-label, and negative-read-back hashes; arbitrary APFS mutation, non-generated APFS file writes, APFS resize, APFS encrypted/compressed files, HFS+ unbounded file/folder create/delete, broad HFS+ allocation growth beyond the bounded initial-extent slice, inline/broad HFS+ attribute growth, compressed-file writes, XFS/Btrfs writes, and deep XFS/Btrfs tool checks remain blocked until operation-specific proof. `tools/filesystem/manifest.json` and the validator prevent any non-native tool or companion artifact from shipping without pinned metadata and matching hashes.
- Guarded generated/minimal APFS raw root-file write/patch/delete, empty root-directory create/delete, root-directory child-file write/patch/delete, and volume-label change helper and certifier routes now have destructive-lab hardware proof and are also exposed through the Partition Manager APFS File action only when the selected APFS partition has S.A.K. generated-layout evidence, exact raw target identity, explicit user confirmation, and Apply review. Non-generated APFS layouts still block the action.
...
- Latest full local automated proof completed on 2026-06-07 PDT / 2026-06-08 UTC: Release build passed, full `ctest --test-dir build -C Release --output-on-failure` passed 136/136, release readiness passed, `git diff --check` passed, physical HFS File Apply proof passed secure released-block zeroing on pinned expendable HFS media at `artifacts\partition-manager-certification\vm-lab\external-evidence\external.hfs-file-apply-physical\report.json`, and APFS raw-format proof passed generated-layout root-file write/patch/delete, empty root-directory create/delete, root-directory child-file write/patch/delete, volume-label change with relabeled root-listing proof, non-empty directory delete blocker, and checksum repair on pinned expendable JMicron APFS media at `artifacts\partition-manager-certification\vm-lab\external-evidence\external.apfs-raw-format\report.json`; latest APFS raw run: `run-20260608-092424`. Current strict hardware proof imports all 18 external VM/hardware/lab gates from `certification/PARTITION_MANAGER_CERTIFICATION_MATRIX.json`, keeps the 12/12 disposable-VHD proof under `artifacts\partition-manager-certification\vhd-strict\`, and reports `HardwareCertified` through the strict hardware handoff using `artifacts\partition-manager-certification\vm-lab\external-evidence.imported.json` plus `external-evidence.imported.checklist.md`.
...
- Added `scripts/check_partition_manager_feature_matrix.ps1` as a release-readiness gate. It verifies 12 commercial-parity groups against implementation, UI, tests, documentation, `certification/PARTITION_MANAGER_CERTIFICATION_MATRIX.json`, and certification gates so feature claims cannot drift from code.
...
| --- | --- | --- | --- |
| Selection right-click menu | Disk, partition, and unallocated rows expose valid operations plus advanced tools with disabled/blocked states. | Added selection-aware table and disk-map context menus with disk actions, partition actions, unallocated actions, Properties, queued operation-backed advanced entries, safety blocker tooltips/status text, and a copyable Properties modal for disk, partition, volume, SMART, flag, and free-space fields. Ribbon and Pending Operations controls stay out of right-click menus. | Keep the certification matrix synchronized with any newly enabled destructive operation. |
...
| Disk map and legend | Bottom map behaves like familiar partition-manager maps: type colors are meaningful, legend matches rendered colors, map/table selection stays synchronized, and row contents align. | Disk-map segments expose GPT/Primary, Logical, Simple, Spanned, Striped, Mirrored, RAID5, and dark-gray Unallocated roles; tests verify every legend swatch renders colored pixels and matches the rendered inner usage-bar color for that role. Segment shells remain neutral except selected partitions, selected whole disks highlight the full disk-map row, disk tiles and partition segments share the same height, and mouse/keyboard/right-click activation on map targets selects the matching table row while table selection updates map highlights. | Keep any new partition role mapped in `partitionColorRole()`, the bottom legend, and the legend-color test before exposing it in the UI. |
...
- Dynamic disks, Storage Spaces mutation, hardware RAID management, arbitrary Linux file system mutation beyond the certified ext/swap scope, and arbitrary Apple APFS/HFS+ partition editing beyond the bounded generated/staged workflows.
...
1. Technician opens Partition Manager, then explicitly clicks Scan Disks before Windows Storage cmdlets enumerate disks, health, partition style, free space, and boot/system markers.
...
- Partitions use color-coded layout bands with labels that truncate safely.
- Selecting a whole disk highlights the full disk-map row and disk tile with the matching layout color.
- Unallocated space uses a dark-gray legend/map color so it cannot read as a blank white segment.
...
- Responsive layout keeps the fixed action rail and resizable center table/map usable at narrow widths.

### Detailed UI Regions

Header toolbar:
- Left: queued Apply/Cancel/Discard/Undo/Redo actions, then stateful Scan Disks/Refresh Disks and Dry Run.
- Disk-count/layout summaries are emitted to the main status bar instead of occupying header space.
- The table and selected-row properties carry model, disk number, size, health, and target details.
- Pending count remains in the fixed left operation rail above the pending-operation queue.
...
- Unallocated space appears as a dark-gray role in the legend and rendered map.
- Selected partitions have strong focus rings, and selected disks highlight the whole disk-map row.
...
- Row context menu exposes target-specific operations and disabled reasons without duplicating ribbon or Pending Operations controls.
...
- Missing volume data is valid for recovery and unformatted partitions; EFI/MSR partitions receive deterministic display labels when Windows returns no mapped volume, and unlabeled hidden/native/non-Windows partitions can receive read-only raw-signature labels when raw access is available.
...
- Refresh SMART data during explicit Scan Disks/Refresh Disks inventory.
...
- Destructive operation evidence covers strict disposable VHD, VM, and imported
  external hardware/lab gates for the shipped scope.
- Release cannot ship unless strict readiness keeps accepting the current
  HardwareCertified evidence set with zero certification gaps.
...
- [x] Complete external manual VM/hardware matrix.
...
- `StorageInventoryWorker` parsing with mocked PowerShell JSON, resilient volume lookup script checks, hidden EFI/MSR file-system fallbacks, and raw-signature detector coverage for native, Linux, HFS+/APFS, and swap labels.
