...
- [~] R5-G20-7 Empty, loading, partial and error states designed for every panel, not
  ...
  - REMAINING (tracked, being finished in a follow-up wave, not deferred): the profile-restore wizard mapping/merge/folder tables; the email folder tree (empty state "Open a PST/OST/MBOX file to browse folders"); the partition disk-map QScrollArea (no model -- needs a container-level placeholder, not the item-view overlay). Also flagged: email_inspector m_search_results_table is declared but NEVER constructed (dead search-results scaffolding) -- surfaced to the owner to finish or remove, not silently deleted.
