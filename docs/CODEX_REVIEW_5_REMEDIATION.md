...
- [~] R5-G20-2 Every long-running action shows progress, is cancellable, and the cancel
  ...
  - REMAINING (bucket B -- worker has NO cancel API, so a truthful "cancel actually stops" needs the work moved off-thread + a cooperative cancel token added first; tracked WITH the blocking-thread item G9-4/G20-4): partition data-recovery/browse-non-native (FileRecoveryEngine), file_explorer disk-scan (StorageInventoryWorker::scanCurrentSystem) + hashFile, backup_restore WindowsUserScanner::scanUsers.
