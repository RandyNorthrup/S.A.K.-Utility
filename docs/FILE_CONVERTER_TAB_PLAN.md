...
|---------|-----------------|---------------|--------|
| Bold / Italic / Underline | [x] | [x] | Core (both engines) |
| Strikethrough | [x] | [x] | Core (both engines) |
| Font name and size | [x] | [x] | Core (both engines) |
| Text color | [x] | [x] | Core (both engines) |
| Background/highlight color | [x] | [x] | Core (both engines) |
| Paragraph alignment | [x] | [x] | Core (both engines) |
| Headings (H1-H6) | [x] | [x] | Core (both engines) |
| Bullet / numbered lists | [x] | [x] | Core (both engines) |
| Basic tables | [x] | [x] | Core (both engines) |
| Page breaks | [x] | [x] | Core (both engines) |
| Line spacing | [x] | [x] | Core (both engines) |
| Embedded images | [x] | [x] | Pandoc (`--extract-media`) |
| Tracked changes | [x] | - | Pandoc (`--track-changes=accept\|reject\|all`) |
| Footnotes / endnotes | [x] | [x] | Pandoc (AST native) |
| Headers / footers | WARNING | WARNING | Pandoc: preserved in DOCX->DOCX via `--reference-doc`; lost in cross-format (not in Pandoc AST) |
...
^2 Qt `QTextDocument` natively loads RTF and renders to PDF, plain text, or HTML. Preserves the same formatting subset as above.  
^3 **Pandoc** (primary): Direct DOCX output with full fidelity. **DocxWriter** (fallback): In-tree writer generates OOXML XML from `QTextDocument`'s block/fragment structure, packages into a ZIP.  
...
| |
|  +--- Log ----------------------------------------------+ |
| | [x] photo.bmp -> photo.webp (4.2 MB -> 312 KB, 92.6%) | |
...
## WARNING Bundled Tool Verification
...
**Acceptance Criteria**:
- [x] Drag-and-drop adds files to the table with auto-detected format
- [x] Target format combo filters to compatible output formats
- [x] Image quality/DPI sliders affect output
- [x] Batch conversion of 100 images completes without errors
- [x] Progress bar and log update in real time
- [x] Cancel stops conversion within 1 second
...
**Acceptance Criteria**:
- [x] PDF -> PNG at 300 DPI produces high-quality page images
- [x] 50 images -> single PDF in correct order
- [x] Single-page TIFF -> single-page PDF with correct DPI and dimensions
- [x] Multi-page TIFF (10 frames) -> 10-page PDF with all frames in order
- [x] 5 multi-page TIFFs combined -> single PDF with correct total page count
- [x] Both `.tiff` and `.tif` extensions handled identically
- [x] PDF merge preserves page content and bookmarks
- [x] Split PDF by page range produces correct individual files
- [x] Text extraction handles multi-column and multi-page PDFs
...
**Acceptance Criteria**:
- [x] CSV with commas, quotes, and newlines in fields parsed correctly
- [x] XLSX output has typed columns (numbers, dates, strings)
- [x] JSON <-> YAML round-trip preserves all data
- [x] XML output is well-formed and indented
- [x] Markdown -> HTML handles headers, lists, code blocks, links
- [x] Encoding conversion handles BOM detection and preservation
...
**Acceptance Criteria**:
- [x] FFmpeg located and verified at startup
- [x] Audio conversion preserves metadata tags
- [x] Video conversion produces playable output in all target formats
- [x] Progress bar updates smoothly during long video conversions
- [x] Video -> GIF produces reasonable quality at configurable size
- [x] Cancel kills FFmpeg process and cleans up partial output files
- [x] Handles missing FFmpeg gracefully (disables audio/video options)
...
**Acceptance Criteria**:
- [x] Pandoc located and verified at startup; graceful degradation if missing
- [x] DOCX -> HTML via Pandoc preserves embedded images, tracked changes, footnotes
- [x] DOCX -> PDF via Pandoc->HTML->QTextDocument produces formatted PDF with images
- [x] DOCX -> PDF via fallback (DocxReader) preserves bold, italic, fonts, colors, headings, tables
- [x] ODT -> HTML/DOCX/RTF/TXT via Pandoc matches DOCX quality
- [x] RTF -> PDF/HTML/DOCX/TXT works via QTextDocument's native RTF support
- [x] Engine fallback works: if Pandoc missing, QTextDocument Hub produces basic output
- [x] EML -> PDF renders header block (From, To, Date, Subject) + HTML body + attachment list
- [x] EML with plain-text-only body produces readable monospace PDF
- [x] EML with multipart/alternative (HTML + plain text) uses HTML for PDF rendering
- [x] Batch EML -> PDF conversion handles 100+ files without errors
- [x] All document conversions work fully offline - zero external dependencies beyond bundled tools
...
**Acceptance Criteria**:
- [x] Generated DOCX files open in Microsoft Word, LibreOffice, and Google Docs
- [x] RTF -> DOCX preserves paragraph and character formatting
- [x] ODT -> DOCX preserves paragraph and character formatting (via Pandoc or DocxWriter)
- [x] TXT -> DOCX produces properly structured document
- [x] Track-changes option correctly accepts, rejects, or preserves changes in DOCX
- [x] Quality presets apply sensible defaults per format
- [x] All collision strategies work correctly
- [x] No external runtime dependencies required for any conversion
...
## [x] Code Quality Gates & Pre-Commit Requirements
