...
zlib compression, A6 credential-gated FileVault encryption, and A7 clones /
sparse files / hard-links / xattr-ACL / in-chunk container resize are each
Apple-certified, and the A8 physical-USB multi-volume + snapshot destructive +
crash + rollback gate closed the APFS track on 2026-06-28. Remaining fail-closed
**by design**: Fusion / multi-device Tier2 (out of scope, no rig); encryption
requires the user credential; a sealed/signed system volume needs a typed
...
proves CLI rename with listing/read-back. Broad catalog B-tree split and
underflow merge/rebalance are now Apple-certified (H2/H3).
...
(`artifacts/file-management-live-certification/disk2-apfs-128mb-raw-format/report.json`)
and the earlier `run-20260612-192652` run remain in the record as the I1-era
...
credential-gated encryption, clones/sparse/hard-links, in-chunk resize, and
volume-label change are approved only through the explicit S.A.K. helper path
with raw-target identity, generated-layout confirmation, and raw hardware
evidence. HFS+/HFSX format and repair are approved through the bundled hfsprogs
sparse-staging path, image-level proof, and physical raw-partition destructive
proof.
...
- APFS full driver-level write (A1-A8, Apple-certified): multi-CIB / CAB space
  ...
  the Partition Manager queue; snapshots (create/delete/revert); multi-volume
...
Remaining enterprise write blockers (everything above is certified):

- APFS: Fusion / multi-device Tier2 containers (out of scope, detect + fail
  closed, no cert rig). By design: encrypted volumes require the user
...
- HFS+/HFSX: none remaining for the driver scope - H1-H8 are Apple-certified
  ...
  follow-ons: catalog map-node growth beyond ~32000 nodes and incremental
  (non-rebuild) B-tree split for efficiency.
...
`fsck_apfs` + macOS-kernel mount.
Release wording must keep XFS/Btrfs to read-only metadata, and may now state
full driver-level APFS + HFS+/HFSX read/write with the standing hard boundaries
(Fusion/Tier2 out of scope; encryption credential-gated; sealed-system-volume
writes only behind a typed seal-invalidation confirmation).
Evidence:
...
- HFS+ folder-tree delete, complex/hard-linked file delete, B-tree split +
...
- APFS write/repair/format, in-place file + directory mutation, snapshots,
  ...
  user credential, and sealed-system-volume writes need a typed
  seal-invalidation confirmation.
