...
- `src/core/ethernet_config_manager.cpp` **runNetsh/restoreSettings/restoreDhcpMode/restoreStaticIp/restoreDnsServers/setDnsServers** and `src/core/app_mutating_actions.cpp` **setAdapterDhcp/StaticIp/Dns** -- elevated netsh rewrites live adapter IPv4/DNS. Cert: `ipv4FieldsWellFormed` + snapshot round-trip unit-tested (`tests/unit/test_ethernet_config_manager.cpp`); live apply is [[no-vm-networking-cert]], certed once on Randy's PC (R3 WAVE F). (Corrected 2026-08-30: this line named `setSourceDhcp`, `applyStaticIp` and `isIpv4Literal`. The first and third exist nowhere in the repo, and `applyStaticIp` lives in `src/gui/network_diagnostic_panel.cpp`, not this file -- so a reader checking the coverage claim would have found nothing to check.)
